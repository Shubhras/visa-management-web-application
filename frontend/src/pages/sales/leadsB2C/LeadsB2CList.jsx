...
import {
  ...
  leadsB2CDelete,
  leadsB2CExportData,
} from "../../../store/sales/leadsB2C/action";
...
const LeadsB2CList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState(
    "Are you sure you want to delete this Leads B2C?"
  );
  ...
  const [departments, setDepartments] = useState([
    ...
    },
    {
      ...
      lastName: "Patel",
      mobileNo: "9090909090",
      emailId: "simran.patel@example.com",
      salesTeam: "Team A",
      salesPerson: "Amit Verma",
    },
  ]);
  ...
  const fetchDepartmentList = () => {
    ...
    dispatch(
      leadsB2CList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all Leads B2C`
        : `${selectedRows.length} selected Leads B2C`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this Leads B2C (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    dispatch(
      leadsB2CDelete(sendPayload, (response, error) => {
        if (error) {
  ...
  const handleExport = () => {
    ...
    dispatch(
      leadsB2CExportData(sendPayload, (response, error) => {
        if (error) {
                        ...
                        departments.map((rowItem, index) => (
                          <tr key={rowItem.uuid}>
                            ...
                            {isColumnVisible("mobileNo") && (
                              <td>
                                <span>{rowItem.mobileNo}</span>
                              </td>
                            )}

                            {isColumnVisible("emailId") && (
                              <td>
                                <span>{rowItem.emailId}</span>
                              </td>
                            )}

                            {isColumnVisible("salesTeam") && (
                              <td>
                                <span>{rowItem.salesTeam}</span>
                              </td>
                            )}

                            {isColumnVisible("salesPerson") && (
                              <td>
                                <span>{rowItem.salesPerson}</span>
                              </td>
                            )}
                ...
                >
                  {departments.map((item, index) => (
                    <div
                      key={item.uuid}
                      style={{
                        flex: "1 0 calc(25% - 16px)", // 4 per row
                    ...
                    >
                      ...
                      <p style={{ margin: "4px 0" }}>
                        <strong>Team:</strong> {item.salesTeam}
                      </p>
                      <p style={{ margin: "4px 0 12px 0" }}>
                        <strong>Sales Person:</strong> {item.salesPerson}
                      </p>
