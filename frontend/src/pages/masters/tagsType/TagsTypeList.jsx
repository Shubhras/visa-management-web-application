...
const TagsTypeList = () => {
  ...
  const fetchTagsTypeList = () => {
    ...
    dispatch(
      tagsTypeList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all tags`
        : `${selectedRows.length} selected tags`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this tags (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    dispatch(
      tagsTypeDelete(sendPayload, (response, error) => {
        if (error) {
          toast.error(error?.response?.data?.message || "server error");
        } else {
          if (response?.statusCode === 200 && response?.status === true) {
            toast.success(response?.message);
            setTagsTypeListData((prevDepts) =>
              prevDepts.filter((dept) => dept.uuid !== deleteId)
            );
  ...
  const handleExport = () => {
    ...
    dispatch(
      tagsTypeExportData(sendPayload, (response, error) => {
        if (error) {
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === tagsTypeListData?.length && (
                      <>
                        <button
