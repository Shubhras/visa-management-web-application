...
import AddTagsType from './AddTagsType';
import EditTagsType from './EditTagsType';
import { tagsTypeList, tagsTypeDelete, tagsTypeExportData } from '../../../store/master/actions';
import AddImportTagsModal from './AddImportTagsModal';
...
const TagsTypeList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this tags?");
  const [showExportPopop, setShowExportPopop] = useState(false);
  const [deleteId, setDeleteId] = useState(null);
  const [selectAllOrNot, setSelectAllOrNot] = useState('');
  const [tagsTypeListData, setTagsTypeListData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [loadingExport, setLoadingExport] = useState(false);
  const [items] = useState(["Tags", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["Tags"]);
  const [ItemsRequired] = useState(["Tags"]);

  const [tableColumns] = useState([
    { id: 'name', label: 'Tags', field: 'name', visible: true, required: false },
  ...
  const fetchTagsTypeList = () => {
    ...
    dispatch(tagsTypeList(params, (response, error) => {
      ...
      if (response?.statusCode === 200 && response?.status === true) {
        ...
        setTagsTypeListData(response?.data || []);
        setTableState(prev => ({
          ...prev,
  ...
  const handleSelectAllButton = () => {
    if (isAllSelected) {
      ...
      setSelectedRows(tagsTypeListData.map(dept => dept.uuid));
    }
  };
  ...
  const isAllSelected = tagsTypeListData.length > 0 &&
    tagsTypeListData.every(dept => selectedRows.includes(dept.uuid));
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this tags?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all tags` : `${selectedRows.length} selected tags`;
    setDeleteConfirmMessage(`Are you sure you want to delete this tags (${message})?`);
    setShowDeleteConfirm(true);
  };
    ...
    dispatch(tagsTypeDelete(sendPayload, (response, error) => {
      if (error) {
        ...
        if (response?.statusCode === 200 && response?.status === true) {
          ...
          setTagsTypeListData(prevDepts => prevDepts.filter(dept => dept.uuid !== deleteId));
  ...
  return (
    <>
      <MasterLayout>
            ...
            <div className="row align-items-center gy-3 gx-2 flex-wrap filter-action-btn">
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {(selectedRows?.length > 0 && selectedRows?.length === tagsTypeListData?.length) && (
                    <>
                      <button
            ...
            <div className='container-table-div'>
              <table className="table mb-0">
                    ...
                    <th scope="col" className='sl-numbar-th'>
                      <div className="d-flex align-items-center gap-2">
                        <input className="form-check-input" type="checkbox" checked={isAllSelected} onChange={handleSelectAll}
                          disabled={tagsTypeListData.length === 0} />
                        <span>No.</span>
                      </div>
                    </th>
                ...
                <tbody>
                  ...
                  ) : tagsTypeListData.length > 0 ? (
                    tagsTypeListData.map((rowItem, index) => (
                      <tr key={rowItem.uuid}>
                        <td>
                          <div className="d-flex align-items-center gap-2">
        ...
        <AddTagsType show={show} handleClose={handleClose} />
        <EditTagsType show={showEdit} handleCloseEdit={handleCloseEdit} rowSelectData={rowSelectData} />
        {showImport && (
          <AddImportTagsModal show={showImport} handleClose={handleCloseImport} />)}
        {showDeleteConfirm && (
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div className="modal-content radius-16 bg-base">
                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                  <h1 className="modal-title fs-5">Export Tags</h1>
                  <button
                    type="button"
                    className="btn-close"
