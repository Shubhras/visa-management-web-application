...
const EmployeeTypeList = () => {
  ...
  const fetchEmployeeTypeList = () => {
    ...
    dispatch(
      employeeTypeList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all employee type`
        : `${selectedRows.length} selected employee type`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this employee type (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  const confirmDelete = () => {
      ...
      employeeTypeDelete(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.statusCode === 200 && response?.status === true) {
            ...
            setEmployeeTypeistData((prevDepts) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      employeeTypeExportData(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.status === 200) {
            ...
            link.href = url;
            link.download = `EmployeeType.xlsx`;
        ...
        <div className="card basic-data-table main-container-data">
          <div className="card-body container-data">
            <div className="row align-items-center gy-3 gx-2 flex-wrap filter-action-btn">
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === employeeTypeistData?.length && (
                      <>
                        <button
              ...
              {/* RIGHT — Page Size / Pagination (NO SEARCH BAR NOW) */}
              ...
              <table className="table mb-0">
                ...
                <tbody>
                  {loading ? (
                    ...
                    employeeTypeistData.map((rowItem, index) => (
                      <tr key={rowItem.uuid}>
                        <td>
                          <div className="d-flex align-items-center gap-2">
                            <input
