...
import { clbLevelList, clbLevelDelete, clbLevelExportData } from '../../../../store/master/testMaster/action';
import AddImportCLBLevelModal from './AddImportCLBLevelModal';
import AddEditCLBLevelModal from './AddEditCLBLevelModal';
...
const CLBLevelList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this clb level?");
    ...
    const [items] = useState(["CLB Level", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["CLB Level"]);
    const [ItemsRequired] = useState(["CLB Level"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'CLB Level', field: 'name', visible: true, required: true },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(clbLevelList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this clb level?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all clb levels` : `${selectedRows.length} selected clb level`;
        setDeleteConfirmMessage(`Are you sure you want to delete this clb level (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No clb level selected for deletion.");
            return;
        }
        dispatch(clbLevelDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(clbLevelExportData(sendPayload, (response, error) => {
            if (error) {
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportCLBLevelModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export CLB Level</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
