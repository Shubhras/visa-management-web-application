...
import { clbLevelImportData } from '../../../../store/master/testMaster/action';
import { toast } from "react-toastify";
import * as XLSX from 'xlsx';
import { saveAs } from "file-saver";
import CommanSampleExcelDownloadModal from '../../../../components/comman/CommanSampleExcelDownloadModal';
const AddImportCLBLevelModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(clbLevelImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate clb level skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
    ...
    const handleExportToExcel = (duplicatesData) => {
        ...
        saveAs(blob, `CLBLevel-Duplicate-Data.xlsx`);
    };
    // Handle modal close
    const onClose = () => {
        setFile(null);
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName:"CLBLevel",
                    items: ["CLB Level", "Description"],
                    selectedItems: ["CLB Level"],
                    ItemsRequired:["CLB Level"]
                }
                } />
            )}
        </>
...
export default AddImportCLBLevelModal;