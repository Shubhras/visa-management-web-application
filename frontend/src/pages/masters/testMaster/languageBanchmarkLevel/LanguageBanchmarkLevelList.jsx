...
import { languageBenchmarkLevelList, languageBenchmarkLevelDelete, languageBenchmarkLevelExportData } from '../../../../store/master/testMaster/action';
import AddImportLanguageBanchmarkLevelModal from './AddImportLanguageBanchmarkLevelModal';
...
const LanguageBanchmarkLevelList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this language banchmark level?");
    ...
    const [items] = useState(["Language Banchmark Level", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Language Banchmark Level"]);
    const [ItemsRequired] = useState(["Language Banchmark Level"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Language Banchmark Level', field: 'name', visible: true, required: false },
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this language banchmark level?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all language banchmark levels` : `${selectedRows.length} selected language banchmark level`;
        setDeleteConfirmMessage(`Are you sure you want to delete this language banchmark level (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No language banchmark level selected for deletion.");
            return;
        }
        dispatch(languageBenchmarkLevelDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(languageBenchmarkLevelExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `LanguageBanchmarkLevel.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditLanguageBanchmarkLevelModal
                ...
                />
                {showImport && (
                    <AddImportLanguageBanchmarkLevelModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Language Banchmark Level</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default LanguageBanchmarkLevelList;