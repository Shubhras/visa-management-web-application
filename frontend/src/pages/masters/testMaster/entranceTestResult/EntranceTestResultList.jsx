...
import { entranceTestResultList, entranceTestResultDelete, entranceTestResultExportData, entranceTestNameList, entranceTestModuleNameList } from '../../../../store/master/testMaster/action';
import AddImportEntranceTestResultModal from './AddImportEntranceTestResultModal';
import AddEditEntranceTestResultModal from './AddEditEntranceTestResultModal';
import { formatDateDDMMYYYY, formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
const EntranceTestResultList = () => {
    ...
    useEffect(() => {
        fetchEntranceTestNameDropdown();
        fetchEntranceTestModuleNameDropdown();
    }, []);

    const fetchEntranceTestNameDropdown = () => {
        const params = { page: 1, limit: 2000, search: "", sortBy: "shortname", sortOrder: "asc" };
        dispatch(entranceTestNameList(params, (response, error) => {
            if (response?.statusCode === 200 && response?.status === true) {
                ...
                setFilterDropdownData(prev => ({
                    ...prev,
                    entranceTestName: sortedOptions,
                }));
            }
        }));
    };

    const fetchEntranceTestModuleNameDropdown = () => {
        const params = { page: 1, limit: 2000, search: "", sortBy: "moduleName", sortOrder: "asc" };
        dispatch(entranceTestModuleNameList(params, (response, error) => {
            if (response?.statusCode === 200 && response?.status === true) {
                ...
                setFilterDropdownData(prev => ({
                    ...prev,
                    entranceTestModuleName: sortedOptions,
                }));
            }
        }));
    };
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this entrance test result?");
    ...
    const [items] = useState(["Entrance Test Name", "Entrance Test Module Name", "Entrance Test Result", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Entrance Test Name", "Entrance Test Module Name", "Entrance Test Result"]);
    const [ItemsRequired] = useState(["Entrance Test Name", "Entrance Test Module Name", "Entrance Test Result"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'shortname', label: 'Entrance Test Name', field: 'entranceTestName', visible: true, required: false, filterable: true },
        { id: 'fullname', label: 'Entrance Test Module Name', field: 'entranceTestModuleName', visible: true, required: false, filterable: true },
        { id: 'result', label: 'Entrance Test Result', field: 'result', visible: true, required: false, filterable: false },
    ...
    const fetchDepartmentList = () => {
        ...
        const params = {
            ...
            entranceTestName: columnFilters.entranceTestName.length > 0 ? columnFilters.entranceTestName : null,
            entranceTestModuleName: columnFilters.entranceTestModuleName.length > 0 ? columnFilters.entranceTestModuleName : null,
        };

        dispatch(entranceTestResultList(params, (response, error) => {
            setLoading(false);
            if (response?.statusCode === 200 && response?.status === true) {
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this entrance test result?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all entrance test result` : `${selectedRows.length} selected entrance test result`;
        setDeleteConfirmMessage(`Are you sure you want to delete this entrance test result (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const handleExport = () => {
        ...
        const fieldMapping = {
            ...
            "Entrance Test Result": "testresult",
        ...
        dispatch(entranceTestResultExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `EntranceTestResult.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditEntranceTestResultModal
                ...
                />
                {showImport && (
                    <AddImportEntranceTestResultModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Entrance Test Result</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default EntranceTestResultList;