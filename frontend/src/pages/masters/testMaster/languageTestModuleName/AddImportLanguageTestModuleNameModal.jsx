...
import { languageTestModuleNameImportData } from '../../../../store/master/testMaster/action';
import { toast } from "react-toastify";
import * as XLSX from 'xlsx';
import { saveAs } from "file-saver";
import CommanSampleExcelDownloadModal from '../../../../components/comman/CommanSampleExcelDownloadModal';
const AddImportLanguageTestModuleNameModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(languageTestModuleNameImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate language test module name skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
    ...
    const handleExportToExcel = (duplicatesData) => {
        ...
        saveAs(blob, `LanguageTestModuleName-Duplicate-Data.xlsx`);
    };
    // Handle modal close
    const onClose = () => {
        setFile(null);
        ...
        <>
                    ...
                    <div className="modal-content radius-16 bg-base">
                        <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                            <h1 className="modal-title fs-5" id="departmentModalLabel">
                                Upload Language Test Module Name
                            </h1>
                            <button
                                type="button"
                                className="btn-close"
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName:"LanguageTestModuleName",
                    items: ["Language Test Module Name", "Description"],
                    selectedItems: ["Language Test Module Name"],
                    ItemsRequired:["Language Test Module Name"]
                }
                } />
            )}
        </>
...
export default AddImportLanguageTestModuleNameModal;