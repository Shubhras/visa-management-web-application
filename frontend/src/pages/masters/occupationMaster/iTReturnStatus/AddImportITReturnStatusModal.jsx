...
import { itReturnStatusImportData } from "../../../../store/master/occupationMaster/action";
...
const AddImportITReturnStatusModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(itReturnStatusImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate it return status skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["IT Return Status"],
                            sheetName: "ITReturnStatus",
                            fileName: "ITReturnStatus",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["IT Return Status", "Description", "Reason"],
                            sheetName: "ITReturnStatus",
                            fileName: "ITReturnStatus",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "ITReturnStatus",
                    items: ["IT Return Status", "Description"],
                    selectedItems: ["IT Return Status"],
                    ItemsRequired: ["IT Return Status"]
                }
                } />
            )}
        </>
...
export default AddImportITReturnStatusModal;