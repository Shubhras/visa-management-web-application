...
import {itReturnStatusList,itReturnStatusDelete,itReturnStatusExportData} from "../../../../store/master/occupationMaster/action";
import AddImportITReturnStatusModal from './AddImportITReturnStatusModal';
import AddEditITReturnStatusModal from './AddEditITReturnStatusModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const ITReturnStatusList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this it return status?");
  ...
  const [items] = useState(["IT Return Status", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["IT Return Status"]);
  const [ItemsRequired] = useState(["IT Return Status"]);

  // Table columns configuration
  const [tableColumns] = useState([
    { id: 'name', label: 'IT Return Status', field: 'name', visible: true, required: false },
  ...
  const fetchDepartmentList = () => {
    ...
    dispatch(itReturnStatusList(params, (response, error) => {
      setLoading(false);
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this it return status?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all it return status` : `${selectedRows.length} selected it return status`;
    setDeleteConfirmMessage(`Are you sure you want to delete this it return status (${message})?`);
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    if (!sendPayload || sendPayload.length === 0) {
      toast.error("No it return statu selected for deletion.");
      return;
    }
    dispatch(itReturnStatusDelete(sendPayload, (response, error) => {
      if (error) {
  ...
  const handleExport = () => {
    ...
    dispatch(itReturnStatusExportData(sendPayload, (response, error) => {
      if (error) {
        ...
        if (response?.status === 200) {
          ...
          link.href = url;
          link.download = `ITReturnStatus.xlsx`;
  ...
  return (
    <>
      <MasterLayout>
        ...
        />
        {showImport && (
          <AddImportITReturnStatusModal show={showImport} handleClose={handleCloseImport} />)}
        {showDeleteConfirm && (
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div className="modal-content radius-16 bg-base">
                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                  <h1 className="modal-title fs-5">Export IT Return Status</h1>
                  <button
                    type="button"
                    className="btn-close"
...
export default ITReturnStatusList;