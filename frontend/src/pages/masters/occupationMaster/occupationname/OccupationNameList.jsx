...
const OccupationNameList = () => {
  ...
  useEffect(() => {
    ...
    fetchOccupationCategoryDropdown();
    fetchOccupationLevelCodeDropdown();
    fetchOccupationLevelDropdown();
  ...
  const fetchOccupationCategoryDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(occupationCategoryList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        const options = (response.data || []).map(item => ({
          id: item.uuid || item.id,
          name: String(item.occupationcategory ?? "")
        }));
        const sortedOptions = options.sort((a, b) =>
          String(a.name).localeCompare(String(b.name), undefined, { numeric: true })
        );
        ...
        setFilterDropdownData(prev => ({
          ...prev,
          occupationCategory: sortedOptions
        }));
      }
    }));
  };
  const fetchOccupationLevelCodeDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(occupationLevelCodeList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        const options = (response.data || []).map(item => ({
          id: item.uuid || item.id,
          name: String(item.occupationlevelcode ?? "")
        }));
        const sortedOptions = options.sort((a, b) =>
          String(a.name).localeCompare(String(b.name), undefined, { numeric: true })
        );
        ...
        setFilterDropdownData(prev => ({
          ...prev,
          occupationLevelCode: sortedOptions
        }));
      }
    }));
  };
  const fetchOccupationLevelDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(occupationLevelList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        const options = (response.data || []).map(item => ({
          id: item.uuid || item.id,
          name: String(item.occupationlevel ?? "")
        }));
        const sortedOptions = options.sort((a, b) =>
          String(a.name).localeCompare(String(b.name), undefined, { numeric: true })
        );
  ...
  const fetchOccupationNameList = () => {
    ...
    const params = {
      ...
      occupationCategory: columnFilters.occupationCategory.length > 0 ? columnFilters.occupationCategory : null,
      occupationLevelCode: columnFilters.occupationLevelCode.length > 0 ? columnFilters.occupationLevelCode : null,
      occupationLevel: columnFilters.occupationLevel.length > 0 ? columnFilters.occupationLevel : null,
  ...
  const confirmDelete = () => {
    ...
    const deleteAll =
      ...
      (
        ...
        columnFilters.occupationCategory.length > 0 ||
        columnFilters.occupationLevelCode.length > 0 ||
        columnFilters.occupationLevel.length > 0 ||
    ...
    const payloadSend = {
      ...
      occupationCategory: columnFilters.occupationCategory.length > 0 ? columnFilters.occupationCategory : '',
      occupationLevelCode: columnFilters.occupationLevelCode.length > 0 ? columnFilters.occupationLevelCode : '',
      occupationLevel: columnFilters.occupationLevel.length > 0 ? columnFilters.occupationLevel : '',
      occupationCode: columnFilters.occupationCode.length > 0 ? columnFilters.occupationCode : '',
      id: deleteAll == true ? "" : sendPayload,
      search: tableState.search || '',
    };
    dispatch(
      occupationNameDelete(payloadSend, (response, error) => {
        if (error) {
