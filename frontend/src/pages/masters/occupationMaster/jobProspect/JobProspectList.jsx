...
const JobProspectList = () => {
  ...
  useEffect(() => {
    ...
    fetchOccupationTypeDropdown();
    fetchOccupationProspectDropdown();
  }, []);
  ...
  const fetchOccupationTypeDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(occupationTypeList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        ...
        setFilterDropdownData(prev => ({
          ...prev,
          occupationType: sortedOptions
        }));
      }
    }));
  };
  const fetchOccupationProspectDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(occupationProspectList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        ...
        setFilterDropdownData(prev => ({
          ...prev,
          occupationProspect: sortedOptions
        }));
      }
    }));
  };
  ...
  const fetchJobProspectList = () => {
    ...
    const params = {
      ...
      occupationType: columnFilters.occupationType.length > 0 ? columnFilters.occupationType : null,
      occupationLevelCode: columnFilters.occupationLevelCode.length > 0 ? columnFilters.occupationLevelCode : null,
      occupationProspect: columnFilters.occupationProspect.length > 0 ? columnFilters.occupationProspect : null,
  ...
  const confirmDelete = () => {
    ...
    const deleteAll =
      ...
      (
        ...
        columnFilters.occupationType.length > 0 ||
        columnFilters.occupationProspect.length > 0
      );
    const payloadSend = {
      deleteAll: deleteAll,
      ...
      occupationType: columnFilters.occupationType.length > 0 ? columnFilters.occupationType : '',
      occupationProspect: columnFilters.occupationProspect.length > 0 ? columnFilters.occupationProspect : '',
      id: deleteAll == true ? "" : sendPayload,
      search: tableState.search || '',
    };
    dispatch(
      jobProspectDelete(payloadSend, (response, error) => {
        if (error) {
