...
import { occupationProspectList, occupationProspectDelete, occupationProspectExportData } from "../../../../store/master/occupationMaster/action";
import AddImportOccupationProspectModal from './AddImportOccupationProspectModal';
import AddEditOccupationProspectModal from './AddEditOccupationProspectModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const OccupationProspectList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this occupation prospect?");
    ...
    const [items] = useState(["Occupation Prospect", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Occupation Prospect"]);
    const [ItemsRequired] = useState(["Occupation Prospect"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Occupation Prospect', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(occupationProspectList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this occupation prospect?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all occupation prospect` : `${selectedRows.length} selected occupation prospects`;
        setDeleteConfirmMessage(`Are you sure you want to delete this occupation prospect (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No occupation prospect selected for deletion.");
            return;
        }
        dispatch(occupationProspectDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(occupationProspectExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `OccupationProspect.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditOccupationProspectModal
                ...
                />
                {showImport && (
                    <AddImportOccupationProspectModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Occupation Prospect</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default OccupationProspectList;