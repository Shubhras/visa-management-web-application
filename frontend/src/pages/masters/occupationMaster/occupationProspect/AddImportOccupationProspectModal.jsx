...
import { occupationProspectImportData } from "../../../../store/master/occupationMaster/action";
...
const AddImportOccupationProspectModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(occupationProspectImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate occupation prospect skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Occupation Prospect"],
                            sheetName: "OccupationProspect",
                            fileName: "OccupationProspect",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Occupation Prospect", "Description", "Reason"],
                            sheetName: "OccupationProspect",
                            fileName: "OccupationProspect",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "OccupationProspect",
                    items: ["Occupation Prospect", "Description"],
                    selectedItems: ["Occupation Prospect"],
                    ItemsRequired: ["Occupation Prospect"]
                }
                } />
            )}
        </>
...
export default AddImportOccupationProspectModal;