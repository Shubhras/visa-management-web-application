...
import {jobTypeImportData} from "../../../../store/master/occupationMaster/action";
...
const AddImportJobTypeModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(jobTypeImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate job type skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Job Type", "Description", "Reason"],
                            sheetName: "JobType",
                            fileName: "JobType",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "JobType",
                    items: ["Job Type", "Description"],
                    selectedItems: ["Job Type"],
                    ItemsRequired: ["Job Type"]
                }
                } />
            )}
        </>
...
export default AddImportJobTypeModal;