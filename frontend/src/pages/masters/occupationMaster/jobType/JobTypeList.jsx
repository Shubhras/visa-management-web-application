...
const JobTypeList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this job type?");
  ...
  const [items] = useState(["Job Type", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["Job Type"]);
  const [ItemsRequired] = useState(["Job Type"]);

  // Table columns configuration
  const [tableColumns] = useState([
    { id: 'name', label: 'Job Type', field: 'name', visible: true, required: false },
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this job type?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all job types` : `${selectedRows.length} selected job types`;
    setDeleteConfirmMessage(`Are you sure you want to delete this job type (${message})?`);
    setShowDeleteConfirm(true);
  };
  ...
  return (
    <>
      <MasterLayout>
        ...
        />
        {showImport && (
          <AddImportJobTypeModal show={showImport} handleClose={handleCloseImport} />)}
        {showDeleteConfirm && (
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div className="modal-content radius-16 bg-base">
                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                  <h1 className="modal-title fs-5">Export Job Type</h1>
                  <button
                    type="button"
                    className="btn-close"
