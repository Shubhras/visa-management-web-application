...
import { modeOfSalaryImportData } from "../../../../store/master/occupationMaster/action";
...
const AddImportModeofSalaryModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(modeOfSalaryImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate mode of salary skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Mode of Salary"],
                            sheetName: "ModeofSalary",
                            fileName: "ModeofSalary",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Mode of Salary", "Description", "Reason"],
                            sheetName: "ModeofSalary",
                            fileName: "ModeofSalary",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "ModeofSalary",
                    items: ["Mode of Salary", "Description"],
                    selectedItems: ["Mode of Salary"],
                    ItemsRequired: ["Mode of Salary"]
                }
                } />
            )}
        </>
...
export default AddImportModeofSalaryModal;