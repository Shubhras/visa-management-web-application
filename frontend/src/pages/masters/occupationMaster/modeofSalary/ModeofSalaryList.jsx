...
import {modeOfSalaryList,modeOfSalaryDelete,modeOfSalaryExportData} from "../../../../store/master/occupationMaster/action";
import AddImportModeofSalaryModal from './AddImportModeofSalaryModal';
import AddEditModeofSalaryModal from './AddEditModeofSalaryModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const ModeofSalaryList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this mode of salary?");
  ...
  const [items] = useState(["Mode of Salary", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["Mode of Salary"]);
  const [ItemsRequired] = useState(["Mode of Salary"]);

  // Table columns configuration
  const [tableColumns] = useState([
    { id: 'name', label: 'Mode of Salary', field: 'name', visible: true, required: false },
  ...
  const fetchDepartmentList = () => {
    ...
    dispatch(modeOfSalaryList(params, (response, error) => {
      setLoading(false);
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this mode of salary?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all mode of salary` : `${selectedRows.length} selected mode of salary`;
    setDeleteConfirmMessage(`Are you sure you want to delete this mode of salary (${message})?`);
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    if (!sendPayload || sendPayload.length === 0) {
      toast.error("No mode of salary selected for deletion.");
      return;
    }
    dispatch(modeOfSalaryDelete(sendPayload, (response, error) => {
      if (error) {
  ...
  const handleExport = () => {
    ...
    dispatch(modeOfSalaryExportData(sendPayload, (response, error) => {
      if (error) {
        ...
        if (response?.status === 200) {
          ...
          link.href = url;
          link.download = `ModeofSalary.xlsx`;
  ...
  return (
    <>
      <MasterLayout>
        ...
        />
        {showImport && (
          <AddImportModeofSalaryModal show={showImport} handleClose={handleCloseImport} />)}
        {showDeleteConfirm && (
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div className="modal-content radius-16 bg-base">
                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                  <h1 className="modal-title fs-5">Export Mode of Salary</h1>
                  <button
                    type="button"
                    className="btn-close"
...
export default ModeofSalaryList;