...
import { instituteTypeList, instituteTypeDelete, instituteTypeExportData } from "../../../../store/master/instituteMaster/action";
import AddImportInstituteTypeModal from './AddImportInstituteTypeModal';
import AddEditInstituteTypeModal from './AddEditInstituteTypeModal';
...
const InstituteTypeList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this institute type?");
    ...
    const [items] = useState(["Institute Type", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Institute Type"]);
    const [ItemsRequired] = useState(["Institute Type"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Institute Type', field: 'name', visible: true, required: false },
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this institute type?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all institute type` : `${selectedRows.length} selected institute type`;
        setDeleteConfirmMessage(`Are you sure you want to delete this institute type (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No institute type selected for deletion.");
            return;
        }
        dispatch(instituteTypeDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(instituteTypeExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `InstituteType.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportInstituteTypeModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Institute Type</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
