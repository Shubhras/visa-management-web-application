...
import { instituteTypeImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportInstituteTypeModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(instituteTypeImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate institute type skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Institute Type"],
                            sheetName: "InstituteType",
                            fileName: "InstituteType",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "InstituteType",
                    items: ["Institute Type", "Description"],
                    selectedItems: ["Institute Type"],
                    ItemsRequired: ["Institute Type"]
                }
                } />
            )}
        </>
...
export default AddImportInstituteTypeModal;