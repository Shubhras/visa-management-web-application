...
import { instituteGroupNameList, instituteGroupNameDelete, instituteGroupNameExportData } from "../../../../store/master/instituteMaster/action";
import AddImportInstituteGroupNameModal from './AddImportInstituteGroupNameModal';
...
const InstituteGroupNameList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this institute group name?");
    ...
    const [items] = useState(["Institute Group Name", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Institute Group Name"]);
    const [ItemsRequired] = useState(["Institute Group Name"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Institute Group Name', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(instituteGroupNameList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this institute group name?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all institute group name` : `${selectedRows.length} selected institute group name`;
        setDeleteConfirmMessage(`Are you sure you want to delete this institute group name (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No institute group name selected for deletion.");
            return;
        }
        dispatch(instituteGroupNameDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(instituteGroupNameExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `InstituteGroupName.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditInstituteGroupNameModal
                ...
                />
                {showImport && (
                    <AddImportInstituteGroupNameModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Institute Group Name</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default InstituteGroupNameList;