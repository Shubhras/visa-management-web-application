...
import { instituteGroupNameImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportInstituteGroupNameModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(instituteGroupNameImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate institute group name skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Institute Group Name"],
                            sheetName: "InstituteGroupName",
                            fileName: "InstituteGroupName",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Institute Group Name", "Reason"],
                            sheetName: "InstituteGroupName",
                            fileName: "InstituteGroupName",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "InstituteGroupName",
                    items: ["Institute Group Name", "Description"],
                    selectedItems: ["Institute Group Name"],
                    ItemsRequired: ["Institute Group Name"]
                }
                } />
            )}
        </>
...
export default AddImportInstituteGroupNameModal;