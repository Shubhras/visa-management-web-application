...
import {
  courseDurationList,
  courseDurationDelete,
  courseDurationExportData,
  courseLevelList,
...
const CourseDurationList = () => {
  ...
  const clearAllOnlyHeaderFilters = () => setColumnFilters({ courseLevel: [] });
  const hasActiveFilters = () =>
    Object.values(columnFilters).some((list) => list.length > 0);
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState(
    "Are you sure you want to delete this Course Duration?"
  );
  ...
  const fetchDepartmentList = () => {
    ...
    const params = {
      ...
      courseLevel:
        columnFilters.courseLevel.length > 0 ? columnFilters.courseLevel : null,
    };
    dispatch(
      courseDurationList(params, (response, error) => {
        setLoading(false);
        if (response?.statusCode === 200 && response?.status === true) {
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all Course Duration`
        : `${selectedRows.length} selected Course Duration`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this Course Duration (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    const payloadSend = {
      deleteAll: deleteAll,
      courseLevel:
        columnFilters.courseLevel.length > 0 ? columnFilters.courseLevel : "",
      id: deleteAll == true ? "" : sendPayload,
      search: tableState.search || "",
    };
    dispatch(
      courseDurationDelete(payloadSend, (response, error) => {
        if (error) {
  ...
  const handleExport = () => {
    ...
    dispatch(
      courseDurationExportData(sendPayload, (response, error) => {
        if (error) {
