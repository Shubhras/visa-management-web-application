...
import { courseDividedInImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportCourseDividedInModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(courseDividedInImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Course Divided In skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Course Divided In"],
                            sheetName: "CourseDividedIn",
                            fileName: "CourseDividedIn",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Course Divided In", "Reason"],
                            sheetName: "CourseDividedIn",
                            fileName: "CourseDividedIn",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "CourseDividedIn",
                    items: ["Course Divided In", "Description"],
                    selectedItems: ["Course Divided In"],
                    ItemsRequired: ["Course Divided In"]
                }
                } />
            )}
        </>
...
export default AddImportCourseDividedInModal;