...
import { bankAccountForImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportBankAccountForModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(bankAccountForImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Bank Account For skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Bank Account For"],
                            sheetName: "BankAccountFor",
                            fileName: "BankAccountFor",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Bank Account For", "Reason"],
                            sheetName: "BankAccountFor",
                            fileName: "BankAccountFor",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "BankAccountFor",
                    items: ["Bank Account For", "Description"],
                    selectedItems: ["Bank Account For"],
                    ItemsRequired: ["Bank Account For"]
                }
                } />
            )}
        </>
...
export default AddImportBankAccountForModal;