...
import { bankAccountForList, bankAccountForDelete, bankAccountForExportData } from "../../../../store/master/instituteMaster/action";
import AddImportBankAccountForModal from './AddImportBankAccountForModal';
import AddEditBankAccountForModal from './AddEditBankAccountForModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
import ResetButton from '../../../../components/comman/ResetButton';
const BankAccountForList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Bank Account For?");
    ...
    const [items] = useState(["Bank Account For", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Bank Account For"]);
    const [ItemsRequired] = useState(["Bank Account For"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Bank Account For', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(bankAccountForList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Bank Account For?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Bank Account For` : `${selectedRows.length} selected Bank Account For`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Bank Account For (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const payloadSend = {
      ...
      search: tableState.search || '',
        ...
        dispatch(bankAccountForDelete(payloadSend, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(bankAccountForExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `BankAccountFor.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportBankAccountForModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Bank Account For</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default BankAccountForList;