...
import { scholarshipBasedOnList, scholarshipBasedOnDelete, scholarshipBasedOnExportData } from "../../../../store/master/instituteMaster/action";
import AddImportScholorshipBasedOnModal from './AddImportScholorshipBasedOnModal';
import AddEditScholorshipBasedOnModal from './AddEditScholorshipBasedOnModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const ScholorshipBasedOnList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Scholorship Based On?");
    ...
    const [items] = useState(["Scholorship Based On", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Scholorship Based On"]);
    const [ItemsRequired] = useState(["Scholorship Based On"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Scholorship Based On', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(scholarshipBasedOnList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Scholorship Based On?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Scholorship Based On` : `${selectedRows.length} selected Scholorship Based On`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Scholorship Based On (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Scholorship Based On selected for deletion.");
            return;
        }
        dispatch(scholarshipBasedOnDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(scholarshipBasedOnExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `ScholorshipBasedOn.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditScholorshipBasedOnModal
                ...
                />
                {showImport && (
                    <AddImportScholorshipBasedOnModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Scholorship Based On</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default ScholorshipBasedOnList;