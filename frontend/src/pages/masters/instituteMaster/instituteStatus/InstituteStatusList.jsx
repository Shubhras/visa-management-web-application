...
import { instituteStatusList, instituteStatusDelete, instituteStatusExportData } from "../../../../store/master/instituteMaster/action";
import AddImportInstituteStatusModal from './AddImportInstituteStatusModal';
...
const InstituteStatusList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this institute status?");
    ...
    const [items] = useState(["Institute Status", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Institute Status"]);
    const [ItemsRequired] = useState(["Institute Status"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Institute Status', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(instituteStatusList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this institute status?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all institute status` : `${selectedRows.length} selected institute status`;
        setDeleteConfirmMessage(`Are you sure you want to delete this institute status (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const handleExport = () => {
        ...
        dispatch(instituteStatusExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `InstituteStatus.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportInstituteStatusModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Institute Status</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default InstituteStatusList;