...
import { instituteStatusImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportInstituteStatusModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(instituteStatusImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate institute status skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Institute Status"],
                            sheetName: "InstituteStatus",
                            fileName: "InstituteStatus",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Institute Status", "Reason"],
                            sheetName: "InstituteStatus",
                            fileName: "InstituteStatus",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "InstituteStatus",
                    items: ["Institute Status", "Description"],
                    selectedItems: ["Institute Status"],
                    ItemsRequired: ["Institute Status"]
                }
                } />
            )}
        </>
...
export default AddImportInstituteStatusModal;