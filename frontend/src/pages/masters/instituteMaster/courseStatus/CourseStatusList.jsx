...
import { courseStatusList, courseStatusDelete, courseStatusExportData } from "../../../../store/master/instituteMaster/action";
import AddImportCourseStatusModal from './AddImportCourseStatusModal';
import AddEditCourseStatusModal from './AddEditCourseStatusModal';
...
const CourseStatusList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Course Status?");
    ...
    const [items] = useState(["Course Status", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Course Status"]);
    const [ItemsRequired] = useState(["Course Status"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Course Status', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(courseStatusList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Course Status?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Course Status` : `${selectedRows.length} selected Course Status`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Course Status (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Course Status selected for deletion.");
            return;
        }
        dispatch(courseStatusDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(courseStatusExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `CourseStatus.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportCourseStatusModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Course Status</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
