...
const CourseLevelList = () => {
  ...
  const fetchCourseLevelCodeDropdown = () => {
    ...
    dispatch(
      courseLevelCodeList(params, (response, error) => {
  ...
  const clearAllOnlyHeaderFilters = () =>
    setColumnFilters({ courseLevelCode: [] });
  ...
  const fetchDepartmentList = () => {
    ...
    const params = {
      ...
      courseLevelCode:
        columnFilters.courseLevelCode.length > 0
          ? columnFilters.courseLevelCode
          : null,
    };
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all Course Level`
        : `${selectedRows.length} selected Course Level`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this Course Level (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    dispatch(
      courseLevelDelete(payloadSend, (response, error) => {
        if (error) {
  ...
  const handleExport = () => {
    ...
    dispatch(
      courseLevelExportData(sendPayload, (response, error) => {
        if (error) {
                    ...
                    departments.map((rowItem, index) => (
                      <tr key={rowItem.uuid}>
                        ...
                        {isColumnVisible("courselevelcode_id") && (
                          <td>
                            <span>{rowItem.courselevelcode}</span>
                          </td>
                        )}
