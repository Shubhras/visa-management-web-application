...
import { institutePriorityImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportInstitutePriorityModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(institutePriorityImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate institute priority skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Institute Priority"],
                            sheetName: "InstitutePriority",
                            fileName: "InstitutePriority",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Institute Priority", "Reason"],
                            sheetName: "InstitutePriority",
                            fileName: "InstitutePriority",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "InstitutePriority",
                    items: ["Institute Priority", "Description"],
                    selectedItems: ["Institute Priority"],
                    ItemsRequired: ["Institute Priority"]
                }
                } />
            )}
        </>
...
export default AddImportInstitutePriorityModal;