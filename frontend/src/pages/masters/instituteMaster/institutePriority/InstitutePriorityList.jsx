...
import { institutePriorityList, institutePriorityDelete, institutePriorityExportData } from "../../../../store/master/instituteMaster/action";
import AddImportInstitutePriorityModal from './AddImportInstitutePriorityModal';
import AddEditInstitutePriorityModal from './AddEditInstitutePriorityModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
const InstitutePriorityList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this institute priority?");
    ...
    const [items] = useState(["Institute Priority", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Institute Priority"]);
    const [ItemsRequired] = useState(["Institute Priority"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Institute Priority', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(institutePriorityList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this institute priority?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all institute priority` : `${selectedRows.length} selected institute priority`;
        setDeleteConfirmMessage(`Are you sure you want to delete this institute priority (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No institute priority selected for deletion.");
            return;
        }
        dispatch(institutePriorityDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(institutePriorityExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `InstitutePriority.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportInstitutePriorityModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Institute Priority</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default InstitutePriorityList;