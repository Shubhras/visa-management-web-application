...
import { instituteDepartmentList, instituteDepartmentDelete, instituteDepartmentExportData } from "../../../../store/master/instituteMaster/action";
import AddImportInstituteDepartmenModal from './AddImportInstituteDepartmentModal';
...
const InstituteDepartmentList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this institute department?");
    ...
    const [items] = useState(["Institute Department", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Institute Department"]);
    const [ItemsRequired] = useState(["Institute Department"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Institute Department', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(instituteDepartmentList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this institute department?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Institute Department` : `${selectedRows.length} selected Institute Department`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Institute Department (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Institute Department selected for deletion.");
            return;
        }
        dispatch(instituteDepartmentDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(instituteDepartmentExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `InstituteDepartment.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditInstituteDepartmenModal
                ...
                />
                {showImport && (
                    <AddImportInstituteDepartmenModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Institute Department</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
