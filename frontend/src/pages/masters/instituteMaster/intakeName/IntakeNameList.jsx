...
import { intakeNameList, intakeNameDelete, intakeNameExportData } from "../../../../store/master/instituteMaster/action";
import AddImportIntakeNameModal from './AddImportIntakeNameModal';
import AddEditIntakeNameModal from './AddEditIntakeNameModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const IntakeNameList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Intake Name?");
    ...
    const [items] = useState(["Intake Name", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Intake Name"]);
    const [ItemsRequired] = useState(["Intake Name"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Intake Name', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(intakeNameList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Intake Name?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Intake Name` : `${selectedRows.length} selected Intake Name`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Intake Name (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Intake Name selected for deletion.");
            return;
        }
        dispatch(intakeNameDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(intakeNameExportData(sendPayload, (response, error) => {
            if (error) {
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportIntakeNameModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Intake Name</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
