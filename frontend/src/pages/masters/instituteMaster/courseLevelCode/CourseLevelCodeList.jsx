...
import { courseLevelCodeList, courseLevelCodeDelete, courseLevelCodeExportData } from "../../../../store/master/instituteMaster/action";
import AddImportCourseLevelCodeModal from './AddImportCourseLevelCodeModal';
import AddEditCourseLevelCodeModal from './AddEditCourseLevelCodeModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
import ResetButton from '../../../../components/comman/ResetButton';
const CourseLevelCodeList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Course Level Code?");
    ...
    const [items] = useState(["Course Level Code", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Course Level Code"]);
    const [ItemsRequired] = useState(["Course Level Code"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Course Level Code', field: 'name', visible: true, required: false },
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Course Level Code?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Course Level Code` : `${selectedRows.length} selected Course Level Code`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Course Level Code (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Course Level Code selected for deletion.");
            return;
        }
    ...
    const payloadSend = {
      ...
      search: tableState.search || '',
        ...
        dispatch(courseLevelCodeDelete(payloadSend, (response, error) => {
            if (error) {
  ...
  const handleCloseImport = (shouldRefresh = false) => {
    ...
    const handleExport = () => {
        ...
        dispatch(courseLevelCodeExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `CourseLevelCode.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportCourseLevelCodeModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Course Level Code</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default CourseLevelCodeList;