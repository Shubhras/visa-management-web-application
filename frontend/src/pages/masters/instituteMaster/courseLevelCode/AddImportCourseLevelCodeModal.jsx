...
import { courseLevelCodeImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportCourseLevelCodeModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(courseLevelCodeImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Course Level Code skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Course Level Code"],
                            sheetName: "CourseLevelCode",
                            fileName: "CourseLevelCode",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Course Level Code", "Reason"],
                            sheetName: "CourseLevelCode",
                            fileName: "CourseLevelCode",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "CourseLevelCode",
                    items: ["Course Level Code", "Description"],
                    selectedItems: ["Course Level Code"],
                    ItemsRequired: ["Course Level Code"]
                }
                } />
            )}
        </>
...
export default AddImportCourseLevelCodeModal;