...
import { courseStatusIntakeImportData } from "../../../../store/master/instituteMaster/action";
...
const AddImportCourseStatusforIntakeModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(courseStatusIntakeImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Course Status for Intake skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Course Status for Intake"],
        ...
        <>
                    ...
                    <div className="modal-content radius-16 bg-base">
                        <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                            <h1 className="modal-title fs-5" id="departmentModalLabel">
                                Upload Course Status for Intake
                            </h1>
                            <button
                                type="button"
                                className="btn-close"
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "CourseStatusforIntake",
                    items: ["Course Status for Intake", "Description"],
                    selectedItems: ["Course Status for Intake"],
                    ItemsRequired: ["Course Status for Intake"]
                }
                } />
            )}
        </>
...
export default AddImportCourseStatusforIntakeModal;