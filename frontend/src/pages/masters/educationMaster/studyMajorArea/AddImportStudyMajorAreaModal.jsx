...
import { studyMajorAreaImportData } from '../../../../store/master/educationMaster/action';
import { toast } from "react-toastify";
import * as XLSX from 'xlsx';
import { saveAs } from "file-saver";
import CommanSampleExcelDownloadModal from '../../../../components/comman/CommanSampleExcelDownloadModal';
const AddImportStudyMajorAreaModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(studyMajorAreaImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate study major area skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
    ...
    const handleExportToExcel = (duplicatesData) => {
        ...
        saveAs(blob, `StudyMajorArea-Duplicate-Data.xlsx`);
    };
    // Handle modal close
    const onClose = () => {
        setFile(null);
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "StudyMajorArea",
                    items: ["Study Main Area", "Study Major Area", "Description"],
                    selectedItems: ["Study Main Area", "Study Major Area"],
                    ItemsRequired: ["Study Main Area", "Study Major Area"]
                }
                } />
            )}
        </>
...
export default AddImportStudyMajorAreaModal;