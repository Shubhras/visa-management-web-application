...
import { academicResultToResultList, academicResultToResultDelete, academicResultToResultExportData, academicResultList, academicResultTypeList } from '../../../../store/master/educationMaster/action';
import AddImportAcademicResultToResultModal from './AddImportAcademicResultToResultModal';
import AddEditAcademicResultToResultModal from './AddEditAcademicResultToResultModal';
...
const AcademicResultToResultList = () => {
    ...
    const fetchAcademicResultTypeDropdown = () => {
        ...
        dispatch(academicResultTypeList(params, (response, error) => {
            if (response?.statusCode === 200 && response?.status === true) {
                ...
                // Fix: Merge with existing data instead of overwriting
    ...
    const fetchAcademicResultDropdown = () => {
        const params = {
            page: 1,
            limit: 2000,
            search: "",
            sortBy: "Academicresult",
            sortOrder: "asc"
        };
        dispatch(academicResultList(params, (response, error) => {
            if (response?.statusCode === 200 && response?.status === true) {
                const options = (response.data || []).map(item => ({
                    id: item.uuid || item.id,
                    name: String(item.Academicresult ?? "")
                }));
                const sortedOptions = options.sort((a, b) =>
                    String(a.name).localeCompare(String(b.name), undefined, { numeric: true })
                );
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this compare : academic result to result?");
    ...
    const [tableColumns] = useState([
        { id: 'AcademicResulttype', label: 'Academic Result Type', field: 'academicResultType', visible: true, required: true, filterable: true },
        { id: 'Academicresult', label: 'Academic Result', field: 'academicResult', visible: true, required: true, filterable: true },
        { id: 'CompareAcademicResultType_id', label: 'Compare : Academic Result Type', field: 'CompareAcademicResultType_id', visible: true, required: true, filterable: false },
        { id: 'CompareAcademicResult', label: 'Compare : Academic Result', field: 'CompareAcademicResult', visible: true, required: true, filterable: false },
    ...
    const fetchDepartmentList = () => {
        ...
        const params = {
            ...
            academicResult: columnFilters.academicResult.length > 0 ? columnFilters.academicResult : null,
        };

        dispatch(academicResultToResultList(params, (response, error) => {
            setLoading(false);
            if (response?.statusCode === 200 && response?.status === true) {
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this compare : academic result to result?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all compare : academic result to results` : `${selectedRows.length} selected compare : academic result to result`;
        setDeleteConfirmMessage(`Are you sure you want to delete this compare : academic result to result (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        dispatch(academicResultToResultDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        const fieldMapping = {
            "Academic Result Type": "AcademicResulttype",
        ...
        dispatch(academicResultToResultExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `Compare:AcademicResultToResult.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                                                                    ...
                                                                    {activeFilterColumn === column.field && (
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <div className="d-flex gap-2 mt-2 pt-2 border-top justify-content-end">
                                                                                <button
                                                                                    className="btn btn-sm  py-1 btn-secondary mt-10"
                                    ...
                                    {loading ? (
                                        ...
                                        departments.map((rowItem, index) => (
                                            <tr key={rowItem.uuid}>
                                                ...
                                                {isColumnVisible('AcademicResulttype') && (
                                                    <td><span>{rowItem.AcademicResulttype}</span></td>
                                                )}
                                                {isColumnVisible('Academicresult') && (
                                                    <td><span>{rowItem.Academicresult}</span></td>
                                                )}
                                                {isColumnVisible('CompareAcademicResultType_id') && (
                                                    <td><span>{rowItem.CompareAcademicResultType_id}</span></td>
                                                )}
                                                {isColumnVisible('CompareAcademicResult') && (
                                                    <td><span>{rowItem.CompareAcademicResult}</span></td>
                                                )}
                ...
                <AddEditAcademicResultToResultModal
                ...
                />
                {showImport && (
                    <AddImportAcademicResultToResultModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Compare : Academic Result To Result</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default AcademicResultToResultList;