...
import { ecaAwardingBodyList, ecaAwardingBodyDelete, ecaAwardingBodyExportData, ecaForList } from '../../../../store/master/educationMaster/action';
import AddImportECAAwardingBodyModal from './AddImportECAAwardingBodyModal';
import AddEditECAAwardingBodyModal from './AddEditECAAwardingBodyModal';
...
const ECAAwardingBodyList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this eca awarding body?");
  ...
  const [items] = useState(["Country", "ECA For", "ECA Body Full Name", "ECA Body Short Name", "ECA Valid Period", "Description", "Modified On"]);
  ...
  const fetchDepartmentList = () => {
    ...
    const params = {
      ...
      ecaFor: (columnFilters.ecaFor?.length > 0) ? columnFilters.ecaFor : null, // Add ? here
    };

    dispatch(ecaAwardingBodyList(params, (response, error) => {
      setLoading(false);
      if (response?.statusCode === 200 && response?.status === true) {
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this eca awarding body?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all eca awarding bodys` : `${selectedRows.length} selected eca awarding bodys`;
    setDeleteConfirmMessage(`Are you sure you want to delete this eca awarding body (${message})?`);
    setShowDeleteConfirm(true);
  };
  ...
  const handleExport = () => {
    ...
    dispatch(ecaAwardingBodyExportData(sendPayload, (response, error) => {
      if (error) {
  ...
  return (
    <>
      <MasterLayout>
                                ...
                                <div className="position-relative comman-filtter-all">
                                  ...
                                  {activeFilterColumn === column.field && (
                                    ...
                                    >
                                      ...
                                      {/* Footer */}
                                      <div className="d-flex gap-2 mt-2 pt-1 border-top justify-content-end">
                                        <button
        ...
        />
        {showImport && (
          <AddImportECAAwardingBodyModal show={showImport} handleClose={handleCloseImport} />)}
        {showDeleteConfirm && (
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div className="modal-content radius-16 bg-base">
                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                  <h1 className="modal-title fs-5">Export ECA Awarding Body</h1>
                  <button
                    type="button"
                    className="btn-close"
...
export default ECAAwardingBodyList;