...
import { academicResultImportData } from '../../../../store/master/educationMaster/action';
import { toast } from "react-toastify";
import * as XLSX from 'xlsx';
import { saveAs } from "file-saver";
import CommanSampleExcelDownloadModal from '../../../../components/comman/CommanSampleExcelDownloadModal';
const AddImportAcademicResultModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(academicResultImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate academic result skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
    ...
    const handleExportToExcel = (duplicatesData) => {
        ...
        saveAs(blob, `AcademicResult-Duplicate-Data.xlsx`);
    };
    // Handle modal close
    const onClose = () => {
        setFile(null);
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "AcademicResult",
                    items: ["Academic Result Type", "Academic Result", "Description"],
                    selectedItems: ["Academic Result Type", "Academic Result"],
                    ItemsRequired: ["Academic Result Type", "Academic Result"]
                }
                } />
            )}
        </>
...
export default AddImportAcademicResultModal;