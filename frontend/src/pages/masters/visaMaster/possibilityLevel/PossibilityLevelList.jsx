...
import AddImportPossibilityLevelModal from './AddImportPossibilityLevelModal';
import AddEditPossibilityLevelModal from './AddEditPossibilityLevelModal';
import { possibilityLevelList, possibilityLevelDelete, possibilityLevelExportData } from '../../../../store/master/visaMaster/action';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
const PossibilityLevelList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Possibility Level?");
    ...
    const [items] = useState(["Possibility Level", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Possibility Level"]);
    const [ItemsRequired] = useState(["Possibility Level"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Possibility Level', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(possibilityLevelList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Possibility Level?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Possibility Level` : `${selectedRows.length} selected Possibility Level`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Possibility Level (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Possibility Level selected for deletion.");
            return;
        }
        dispatch(possibilityLevelDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(possibilityLevelExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `PossibilityLevel.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportPossibilityLevelModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Possibility Level</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default PossibilityLevelList;