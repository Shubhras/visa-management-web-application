...
import { possibilityLevelImportData } from '../../../../store/master/visaMaster/action';
import { exportToExcelDuplicate, exportToExcelWrongData } from '../../../../helper/utils/commanHelper';
const AddImportPossibilityLevelModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(possibilityLevelImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Possibility Level skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Possibility Level"],
                            sheetName: "PossibilityLevel",
                            fileName: "PossibilityLevel",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Possibility Level", "Reason"],
                            sheetName: "PossibilityLevel",
                            fileName: "PossibilityLevel",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "PossibilityLevel",
                    items: ["Possibility Level", "Description"],
                    selectedItems: ["Possibility Level"],
                    ItemsRequired: ["Possibility Level"]
                }
                } />
            )}
        </>
...
export default AddImportPossibilityLevelModal;