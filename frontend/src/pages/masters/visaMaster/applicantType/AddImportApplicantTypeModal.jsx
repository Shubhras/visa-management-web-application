...
import { applicantTypeImportData } from '../../../../store/master/visaMaster/action';
import { exportToExcelDuplicate, exportToExcelWrongData } from '../../../../helper/utils/commanHelper';
const AddImportApplicantTypeModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(applicantTypeImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Applicant Type skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Applicant Type"],
                            sheetName: "ApplicantType",
                            fileName: "ApplicantType",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "ApplicantType",
                    items: ["Applicant Type", "Description"],
                    selectedItems: ["Applicant Type"],
                    ItemsRequired: ["Applicant Type"]
                }
                } />
            )}
        </>
...
export default AddImportApplicantTypeModal;