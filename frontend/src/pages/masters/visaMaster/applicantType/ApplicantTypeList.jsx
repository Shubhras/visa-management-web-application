...
import AddImportApplicantTypeModal from './AddImportApplicantTypeModal';
import AddEditApplicantTypeModal from './AddEditApplicantTypeModal';
import { applicantTypeList, applicantTypeDelete, applicantTypeExportData } from '../../../../store/master/visaMaster/action';
...
const ApplicantTypeList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Applicant Type?");
    ...
    const [items] = useState(["Applicant Type", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Applicant Type"]);
    const [ItemsRequired] = useState(["Applicant Type"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Applicant Type', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(applicantTypeList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Applicant Type?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Applicant Type` : `${selectedRows.length} selected Applicant Type`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Applicant Type (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Applicant Type selected for deletion.");
            return;
        }
        dispatch(applicantTypeDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(applicantTypeExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `ApplicantType.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportApplicantTypeModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Applicant Type</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default ApplicantTypeList;