...
import { visaEligibilityTypeImportData } from '../../../../store/master/visaMaster/action';
import { exportToExcelDuplicate, exportToExcelWrongData } from '../../../../helper/utils/commanHelper';
const AddImportVisaEligibilityTypeModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(visaEligibilityTypeImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Visa Eligibility Type skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Visa Eligibility Type"],
                            sheetName: "VisaEligibilityType",
                            fileName: "VisaEligibilityType",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Visa Eligibility Type", "Reason"],
                            sheetName: "VisaEligibilityType",
                            fileName: "VisaEligibilityType",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "VisaEligibilityType",
                    items: ["Visa Eligibility Type", "Description"],
                    selectedItems: ["Visa Eligibility Type"],
                    ItemsRequired: ["Visa Eligibility Type"]
                }
                } />
            )}
        </>
...
export default AddImportVisaEligibilityTypeModal;