...
import AddImportVisaEligibilityTypeModal from './AddImportVisaEligibilityTypeModal';
import AddEditVisaEligibilityTypeModal from './AddEditVisaEligibilityTypeModal';
import { visaEligibilityTypeList, visaEligibilityTypeDelete, visaEligibilityTypeExportData } from '../../../../store/master/visaMaster/action';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
const VisaEligibilityTypeList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Visa Eligibility Type?");
    ...
    const [items] = useState(["Visa Eligibility Type", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Visa Eligibility Type"]);
    const [ItemsRequired] = useState(["Visa Eligibility Type"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Visa Eligibility Type', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(visaEligibilityTypeList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Visa Eligibility Type?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Visa Eligibility Type` : `${selectedRows.length} selected Visa Eligibility Type`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Visa Eligibility Type (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Visa Eligibility Type selected for deletion.");
            return;
        }
        dispatch(visaEligibilityTypeDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(visaEligibilityTypeExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `VisaEligibilityType.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditVisaEligibilityTypeModal
                ...
                />
                {showImport && (
                    <AddImportVisaEligibilityTypeModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Visa Eligibility Type</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default VisaEligibilityTypeList;