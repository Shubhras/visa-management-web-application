...
import { visaNameImportData } from '../../../../store/master/visaMaster/action';
import { exportToExcelDuplicate, exportToExcelWrongData } from '../../../../helper/utils/commanHelper';
const AddImportVisaNameModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(visaNameImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Visa Name skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Country", "Visa Main Category", "Visa Major Category", "Visa Name"],
                            sheetName: "VisaName",
                            fileName: "VisaName",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Country", "Visa Main Category", "Visa Major Category", "Visa Name", "Reason"],
                            sheetName: "VisaName",
                            fileName: "VisaName",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "VisaName",
                    items: ["Country", "Visa Main Category", "Visa Major Category", "Visa Name", "Visa Short Name", "Description"],
                    selectedItems: ["Country", "Visa Main Category", "Visa Major Category", "Visa Name"],
                    ItemsRequired: ["Country", "Visa Main Category", "Visa Major Category", "Visa Name"]
                }
                } />
            )}
        </>
...
export default AddImportVisaNameModal;