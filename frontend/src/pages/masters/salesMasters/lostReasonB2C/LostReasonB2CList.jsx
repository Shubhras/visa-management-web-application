...
const LostReasonB2CList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState(
    "Are you sure you want to delete this lost reason (B2C)?"
  );
  ...
  const fetchLostReasonB2CList = () => {
    ...
    dispatch(
      lostReasonB2CList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all lost reason (B2C)`
        : `${selectedRows.length} selected lost reason (B2C)`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this lost reason (B2C) (${message})?`
    );
    setShowDeleteConfirm(true);
  };
      ...
      lostReasonB2CDelete(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.statusCode === 200 && response?.status === true) {
            ...
            setLostReasonB2CListData((prevDepts) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      lostReasonB2CExportData(sendPayload, (response, error) => {
        if (error) {
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === lostReasonB2CListData?.length && (
                      <>
                        <button
