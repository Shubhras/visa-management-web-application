...
const LostReasonB2CList = () => {
  ...
  const handleBulkDelete = () => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this lost reason B2C (${maggase})?`);
    setShowDeleteConfirm(true);
  };
    ...
    dispatch(lostReasonB2CExportData(sendPayload, (response, error) => {
      if (error) {
        ...
        if (response?.status === 200) {
          ...
          link.href = url;
          link.download = `lostReasonB2CListData_${new Date().toISOString().split('T')[0]}.csv`;
