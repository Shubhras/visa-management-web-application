...
const ActivityTypeList = () => {
  ...
  const fetchActivityTypeList = () => {
    ...
    dispatch(
      activityTypeList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all activity type`
        : `${selectedRows.length} selected activity type`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this activity type (${message})?`
    );
    setShowDeleteConfirm(true);
  };
      ...
      activityTypeDelete(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.statusCode === 200 && response?.status === true) {
            ...
            setActivityTypeListData((prevDepts) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      activityTypeExportData(sendPayload, (response, error) => {
        if (error) {
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === activityTypeListData?.length && (
                      <>
                        <button
