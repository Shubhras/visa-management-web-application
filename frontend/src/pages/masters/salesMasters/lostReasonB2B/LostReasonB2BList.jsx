...
const LostReasonB2BList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState(
    "Are you sure you want to delete this lost reason B2B?"
  );
  ...
  const [items] = useState(["Lost Reason (B2B)", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["Lost Reason (B2B)"]);
  const [ItemsRequired] = useState(["Lost Reason (B2B)"]);

  const [tableColumns] = useState([
    {
      id: "name",
      label: "Lost Reason (B2B)",
      field: "name",
      visible: true,
      required: false,
    },
  ...
  const fetchLostReasonB2BList = () => {
    ...
    dispatch(
      lostReasonB2BList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all lost reason (B2B)`
        : `${selectedRows.length} selected lost reason (B2B)`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this lost reason (B2B) (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    if (!sendPayload || sendPayload.length === 0) {
      toast.error("No lost reason (B2B) selected for deletion.");
      return;
    }

    dispatch(
      lostReasonB2BDelete(sendPayload, (response, error) => {
        if (error) {
          toast.error(error?.response?.data?.message || "server error");
        } else {
          if (response?.statusCode === 200 && response?.status === true) {
            toast.success(response?.message);
            setLostReasonB2BListData((prevDepts) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      lostReasonB2BExportData(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.status === 200) {
            ...
            link.href = url;
            link.download = `LostReason(B2B).xlsx`;
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === lostReasonB2BListData?.length && (
                      <>
                        <button
              ...
              <table className="table mb-0">
                ...
                <tbody>
                  {loading ? (
                    ...
                    lostReasonB2BListData.map((rowItem, index) => (
                      <tr key={rowItem.uuid}>
                        <td>
                          <div className="d-flex align-items-center gap-2">
                            <input
