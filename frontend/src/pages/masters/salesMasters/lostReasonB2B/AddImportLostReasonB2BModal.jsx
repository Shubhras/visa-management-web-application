...
import { lostReasonB2BImportData } from '../../../../store/master/salesMasters/actions';
import { toast } from "react-toastify";
import * as XLSX from 'xlsx';
import { saveAs } from "file-saver";
const AddImportLostReasonB2BModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(lostReasonB2BImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate lost reason (B2B) skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
    ...
    const handleExportToExcel = (duplicatesData) => {
        ...
        saveAs(blob, `LostReason(B2B)_${new Date().toISOString().split("T")[0]}.xlsx`);
    };
    // Handle modal close
    const onClose = () => {
        setFile(null);
    ...
    const handleDownloadSample = () => {
        const fileUrl = 'assets/simplefile/LostReason(B2B).xlsx'; // Update this path according to your project structure

        const link = document.createElement('a');
        link.href = fileUrl;
        link.download = 'LostReason(B2B).xlsx'; // Downloaded file name
    ...
    return (
        <div
            ...
            tabIndex={-1}
            role="dialog"
            aria-labelledby="LostReasonB2BModalLabel"
            aria-hidden={!show}
            ...
            <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div className="modal-content radius-16 bg-base">
                    <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                        <h1 className="modal-title fs-5" id="LostReasonB2BModalLabel">
                            Upload Lost Reason (B2B)
                        </h1>
                        <button
                            type="button"
                            className="btn-close"
...
export default AddImportLostReasonB2BModal;