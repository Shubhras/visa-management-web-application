...
import { spouseVisaCategoryImportData } from "../../../../store/master/visaConditionsMaster/action";
...
const AddImportSpouseVisaCategoryModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(spouseVisaCategoryImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Spouse Visa Category skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Spouse Visa Category"],
                            sheetName: "SpouseVisaCategory",
                            fileName: "SpouseVisaCategory",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Spouse Visa Category", "Reason"],
                            sheetName: "SpouseVisaCategory",
                            fileName: "SpouseVisaCategory",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "SpouseVisaCategory",
                    items: ["Spouse Visa Category", "Description"],
                    selectedItems: ["Spouse Visa Category"],
                    ItemsRequired: ["Spouse Visa Category"]
                }
                } />
            )}
        </>
...
export default AddImportSpouseVisaCategoryModal;