...
import { childrenApplyWithCandidateImportData } from "../../../../store/master/visaConditionsMaster/action";
...
const AddImportChildrenCanApplywithCandidateModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(childrenApplyWithCandidateImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Children Can Apply with Candidate skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Children Can Apply with Candidate"],
                            sheetName: "ChildrenCanApplywithCandidate",
                            fileName: "ChildrenCanApplywithCandidate",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Children Can Apply with Candidate", "Reason"],
                            sheetName: "ChildrenCanApplywithCandidate",
                            fileName: "ChildrenCanApplywithCandidate",
                        };
        ...
        <>
                    ...
                    <div className="modal-content radius-16 bg-base">
                        <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                            <h1 className="modal-title fs-5" id="departmentModalLabel">
                                Upload Children Can Apply with Candidate
                            </h1>
                            <button
                                type="button"
                                className="btn-close"
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "ChildrenCanApplywithCandidate",
                    items: ["Children Can Apply with Candidate", "Description"],
                    selectedItems: ["Children Can Apply with Candidate"],
                    ItemsRequired: ["Children Can Apply with Candidate"]
                }
                } />
            )}
        </>
...
export default AddImportChildrenCanApplywithCandidateModal;