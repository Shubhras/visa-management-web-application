...
import { visaMainCategoryImportData } from "../../../../store/master/visaConditionsMaster/action";
...
const AddImportVisaMainCategoryModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(visaMainCategoryImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Visa Main Category skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Visa Main Category"],
                            sheetName: "VisaMainCategory",
                            fileName: "VisaMainCategory",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Visa Main Category", "Reason"],
                            sheetName: "VisaMainCategory",
                            fileName: "VisaMainCategory",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "VisaMainCategory",
                    items: ["Visa Main Category", "Description"],
                    selectedItems: ["Visa Main Category"],
                    ItemsRequired: ["Visa Main Category"]
                }
                } />
            )}
        </>
...
export default AddImportVisaMainCategoryModal;