...
import { prPossibilityList, prPossibilityDelete, prPossibilityExportData } from "../../../../store/master/visaConditionsMaster/action";
import AddImportPRPossibilityModal from './AddImportPRPossibilityModal';
import AddEditPRPossibilityModal from './AddEditPRPossibilityModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
import ResetButton from '../../../../components/comman/ResetButton';
const PRPossibilityList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this  PR Possibility?");
    ...
    const [items] = useState(["PR Possibility", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["PR Possibility"]);
    const [ItemsRequired] = useState([" PR Possibility"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'PR Possibility', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(prPossibilityList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this PR Possibility?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all PR Possibility` : `${selectedRows.length} selected PR Possibility`;
        setDeleteConfirmMessage(`Are you sure you want to delete this PR Possibility (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No PR Possibility selected for deletion.");
            return;
        }
        dispatch(prPossibilityDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(prPossibilityExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `PRPossibility.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportPRPossibilityModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export PR Possibility</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default PRPossibilityList;