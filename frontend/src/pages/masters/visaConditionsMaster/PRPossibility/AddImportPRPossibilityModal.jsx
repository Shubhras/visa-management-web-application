...
import { prPossibilityImportData } from "../../../../store/master/visaConditionsMaster/action";
...
const AddImportPRPossibilityModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(prPossibilityImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate PR Possibility skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["PR Possibility"],
                            sheetName: "PRPossibility",
                            fileName: "PRPossibility",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "PRPossibility",
                    items: ["PR Possibility", "Description"],
                    selectedItems: ["PR Possibility"],
                    ItemsRequired: ["PR Possibility"]
                }
                } />
            )}
        </>
...
export default AddImportPRPossibilityModal;