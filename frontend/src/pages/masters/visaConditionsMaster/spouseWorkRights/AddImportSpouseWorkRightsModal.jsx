...
import { spouseWorkRightsImportData } from "../../../../store/master/visaConditionsMaster/action";
...
const AddImportSpouseWorkRightsModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(spouseWorkRightsImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Spouse Work Rights skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Spouse Work Rights"],
                            sheetName: "SpouseWorkRights",
                            fileName: "SpouseWorkRights",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Spouse Work Rights", "Reason"],
                            sheetName: "SpouseWorkRights",
                            fileName: "SpouseWorkRights",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "SpouseWorkRights",
                    items: ["Spouse Work Rights", "Description"],
                    selectedItems: ["Spouse Work Rights"],
                    ItemsRequired: ["Spouse Work Rights"]
                }
                } />
            )}
        </>
...
export default AddImportSpouseWorkRightsModal;