...
import { spouseWorkRightsList, spouseWorkRightsDelete, spouseWorkRightsExportData } from "../../../../store/master/visaConditionsMaster/action";
import AddImportSpouseWorkRightsModal from './AddImportSpouseWorkRightsModal';
import AddEditSpouseWorkRightsModal from './AddEditSpouseWorkRightsModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const SpouseWorkRightsList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Spouse Work Rights?");
    ...
    const [items] = useState(["Spouse Work Rights", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Spouse Work Rights"]);
    const [ItemsRequired] = useState(["Spouse Work Rights"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Spouse Work Rights', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(spouseWorkRightsList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Spouse Work Rights?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Spouse Work Rights` : `${selectedRows.length} selected Spouse Work Rights`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Spouse Work Rights (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const handleExport = () => {
        ...
        dispatch(spouseWorkRightsExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `SpouseWorkRights.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportSpouseWorkRightsModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Spouse Work Rights</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default SpouseWorkRightsList;