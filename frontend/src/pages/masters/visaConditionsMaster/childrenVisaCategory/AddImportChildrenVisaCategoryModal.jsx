...
import { childrenVisaCategoryImportData } from "../../../../store/master/visaConditionsMaster/action";
...
const AddImportChildrenVisaCategoryModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(childrenVisaCategoryImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Children Visa Category skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Children Visa Category"],
                            sheetName: "ChildrenVisaCategory",
                            fileName: "ChildrenVisaCategory",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Children Visa Category", "Reason"],
                            sheetName: "ChildrenVisaCategory",
                            fileName: "ChildrenVisaCategory",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "ChildrenVisaCategory",
                    items: ["Children Visa Category", "Description"],
                    selectedItems: ["Children Visa Category"],
                    ItemsRequired: ["Children Visa Category"]
                }
                } />
            )}
        </>
...
export default AddImportChildrenVisaCategoryModal;