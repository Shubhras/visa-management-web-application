...
import { childrenVisaCategoryList, childrenVisaCategoryDelete, childrenVisaCategoryExportData } from "../../../../store/master/visaConditionsMaster/action";
import AddImportChildrenVisaCategoryModal from './AddImportChildrenVisaCategoryModal';
import AddEditChildrenVisaCategoryModal from './AddEditChildrenVisaCategoryModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const ChildrenVisaCategoryList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Children Visa Category?");
    ...
    const [items] = useState(["Children Visa Category", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Children Visa Category"]);
    const [ItemsRequired] = useState(["Children Visa Category"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Children Visa Category', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(childrenVisaCategoryList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Children Visa Category?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Children Visa Category` : `${selectedRows.length} selected Children Visa Category`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Children Visa Category (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Children Visa Category selected for deletion.");
            return;
        }
        dispatch(childrenVisaCategoryDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(childrenVisaCategoryExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `ChildrenVisaCategory.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditChildrenVisaCategoryModal
                ...
                />
                {showImport && (
                    <AddImportChildrenVisaCategoryModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Children Visa Category</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default ChildrenVisaCategoryList;