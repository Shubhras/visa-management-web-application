...
import { childrenStudyWorkRightsList, childrenStudyWorkRightsDelete, childrenStudyWorkRightsExportData } from "../../../../store/master/visaConditionsMaster/action";
import AddImportChildrenStudyWorkRightsModal from './AddImportChildrenStudyWorkRightsModal';
import AddEditChildrenStudyWorkRightsModal from './AddEditChildrenStudyWorkRightsModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';
import { useGlobalSearch } from '../../../../components/comman/GlobalSearchContext';
import ResetButton from '../../../../components/comman/ResetButton';
const ChildrenStudyWorkRightsList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Children Study / Work Rights?");
    ...
    const [items] = useState(["Children Study / Work Rights", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Children Study / Work Rights"]);
    const [ItemsRequired] = useState(["Children Study / Work Rights"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Children Study / Work Rights', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(childrenStudyWorkRightsList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Children Study / Work Rights?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Children Study / Work Rights` : `${selectedRows.length} selected Children Study / Work Rights`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Children Study / Work Rights (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Children Study / Work Rights selected for deletion.");
            return;
        }
        dispatch(childrenStudyWorkRightsDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        const fieldMapping = {
            "Children Study / Work Rights": "name",
        ...
        dispatch(childrenStudyWorkRightsExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `ChildrenStudy/WorkRights.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditChildrenStudyWorkRightsModal
                ...
                />
                {showImport && (
                    <AddImportChildrenStudyWorkRightsModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Children Study / Work Rights</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default ChildrenStudyWorkRightsList;