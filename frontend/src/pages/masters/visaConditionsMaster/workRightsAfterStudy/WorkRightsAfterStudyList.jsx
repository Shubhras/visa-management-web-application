...
import { workRightsAfterStudyList, workRightsAfterStudyDelete, workRightsAfterStudyExportData } from "../../../../store/master/visaConditionsMaster/action";
import AddImportWorkRightsAfterStudyModal from './AddImportWorkRightsAfterStudyModal';
import AddEditWorkRightsAfterStudyModal from './AddEditWorkRightsAfterStudyModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const WorkRightsAfterStudyList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Work Rights After Study?");
    ...
    const [items] = useState(["Work Rights After Study", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Work Rights After Study"]);
    const [ItemsRequired] = useState(["Work Rights After Study"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Work Rights After Study', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(workRightsAfterStudyList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Work Rights After Study?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Work Rights After Study` : `${selectedRows.length} selected Work Rights After Study`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Work Rights After Study (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Work Rights After Study selected for deletion.");
            return;
        }
        dispatch(workRightsAfterStudyDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(workRightsAfterStudyExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `WorkRightsAfterStudy.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditWorkRightsAfterStudyModal
                ...
                />
                {showImport && (
                    <AddImportWorkRightsAfterStudyModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Work Rights After Study</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default WorkRightsAfterStudyList;