...
import { workRightsAfterStudyImportData } from "../../../../store/master/visaConditionsMaster/action";
...
const AddImportWorkRightsDuringVacationModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(workRightsAfterStudyImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Work Rights After Study skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Work Rights After Study"],
                            sheetName: "WorkRightsAfterStudy",
                            fileName: "WorkRightsAfterStudy",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Work Rights After Study", "Reason"],
                            sheetName: "WorkRightsAfterStudy",
                            fileName: "WorkRightsAfterStudy",
                        };
        ...
        <>
                    ...
                    <div className="modal-content radius-16 bg-base">
                        <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                            <h1 className="modal-title fs-5" id="departmentModalLabel">
                                Upload Work Rights After Study
                            </h1>
                            <button
                                type="button"
                                className="btn-close"
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "WorkRightsAfterStudy",
                    items: ["Work Rights After Study", "Description"],
                    selectedItems: ["Work Rights After Study"],
                    ItemsRequired: ["Work Rights After Study"]
                }
                } />
            )}
        </>
