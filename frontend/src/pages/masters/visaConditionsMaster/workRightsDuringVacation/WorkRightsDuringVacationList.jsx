...
import { workRightsDuringVacationList, workRightsDuringVacationDelete, workRightsDuringVacationExportData } from "../../../../store/master/visaConditionsMaster/action";
import AddImportWorkRightsDuringVacationModal from './AddImportWorkRightsDuringVacationModal';
import AddEditWorkRightsDuringVacationModal from './AddEditWorkRightsDuringVacationModal';
import { formatDateDDMMYYYYTime } from '../../../../helper/utils/commanHelper';

const WorkRightsDuringVacationList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Work Rights During Vacation?");
    ...
    const [items] = useState(["Work Rights During Vacation", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Work Rights During Vacation"]);
    const [ItemsRequired] = useState(["Work Rights During Vacation"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Work Rights During Vacation', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(workRightsDuringVacationList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Work Rights During Vacation?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Work Rights During Vacation` : `${selectedRows.length} selected Work Rights During Vacation`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Work Rights During Vacation (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Work Rights During Vacation selected for deletion.");
            return;
        }
        dispatch(workRightsDuringVacationDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        const fieldMapping = {
            "Work Rights During Vacation": "name",
        ...
        dispatch(workRightsDuringVacationExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `WorkRightsDuringVacation.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                <AddEditWorkRightsDuringVacationModal
                ...
                />
                {showImport && (
                    <AddImportWorkRightsDuringVacationModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Work Rights During Vacation</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default WorkRightsDuringVacationList;