...
const MaritalStatusList = () => {
  ...
  const fetchMaritalStatusList = () => {
    ...
    dispatch(
      maritalStatusList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all marital status`
        : `${selectedRows.length} selected marital status`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this marital status (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    dispatch(
      maritalStatusDelete(payloadSend, (response, error) => {
        if (error) {
          ...
          if (typeof response === "string") {
            ...
            link.href = url;
            link.download = "MaritalStatusDependencyReport.xlsx";
  ...
  const handleExport = () => {
    ...
    dispatch(
      maritalStatusExportData(sendPayload, (response, error) => {
        if (error) {
