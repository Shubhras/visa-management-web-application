...
const TimeZoneList = () => {
  ...
  const fetchTimeZoneList = () => {
    ...
    dispatch(
      timeZoneList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all time zone`
        : `${selectedRows.length} selected time zone`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this time zone (${message})?`
    );
    setShowDeleteConfirm(true);
  };
      ...
      timeZoneDelete(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.statusCode === 200 && response?.status === true) {
            ...
            setTimeZoneDataList((prevRowItems) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      timeZoneExportData(sendPayload, (response, error) => {
        if (error) {
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === timeZoneDataList?.length && (
                      <>
                        <button
