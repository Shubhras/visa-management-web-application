...
const StateList = () => {
  ...
  const fetchDepartmentList = () => {
    ...
    dispatch(
      stateList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all state`
        : `${selectedRows.length} selected state`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this state (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const handleExport = () => {
    ...
    dispatch(
      stateExportData(sendPayload, (response, error) => {
        if (error) {
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === stateListData?.length && (
                      <>
                        <button
                    ...
                    stateListData.map((rowItem, index) => (
                      <tr key={rowItem.uuid}>
                        ...
                        {isColumnVisible("stateshortName") && (
                          <td>
                            <span>{rowItem.stateshortName}</span>
                          </td>
                        )}
                        {isColumnVisible("state_display") && (
                          <td>
                            <span>{rowItem.state_display}</span>
                          </td>
                        )}
