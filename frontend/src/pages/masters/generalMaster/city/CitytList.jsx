...
const CityList = () => {
  ...
  const [tableColumns] = useState([
    { id: 'countryName', label: 'Country Name', field: 'countryName', visible: true, required: false, filterable: true },
    { id: 'stateName', label: 'State Name', field: 'stateName', visible: true, required: false, filterable: true },
    { id: 'districtName', label: 'District Name ', field: 'districtName', visible: true, required: false, filterable: true },
    { id: 'cityName', label: 'City Name', field: 'cityName', visible: true, required: false, filterable: true },
  ...
  const extractFilterOptions = (data, field) => {
  ...
  const fetchCityList = () => {
    ...
    dispatch(cityList(params, (response, error) => {
      ...
      if (response?.statusCode === 200 && response?.status === true) {
        ...
        // Extract filter options for all filterable columns
        const filterData = {};
        tableColumns.forEach(col => {
          if (col.filterable) {
            filterData[col.field] = extractFilterOptions(data, col.field);
          }
        });
        setFilterDropdownData(filterData);
  ...
  const handleFilterCheckboxChange = (columnField, value, checked) => {
    ...
    console.log('hhhhhhhhhhhhhhhh', value);
  ...
  const handleFilterSelectAll = (columnField) => {
    ...
    const availableOptions = (filterDropdownData[columnField] || []).filter(option =>
                ...
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {hasActiveFilters() && (
                    <button
                      ...
                      className="btn btn-sm py-1 btn-outline-secondary"
                    ...
                    {tableColumns.map((column) => (
                      isColumnVisible(column.id) && (
                        <th
                          ...
                          className='sorting-th'
                        >
                          <div className="d-flex align-items-center justify-content-between position-relative nagar-test">
                            <div
                            ...
                            {column.filterable && (
                              <div className="position-relative comman-filtter-all">
                                <Icon
                                  ...
                                  width="18"
                                  className={`ms-2 ${columnFilters[column.field]?.length > 0 ? 'text-primary' : ''}`}
                                  style={{ cursor: 'pointer' }}
                                  ...
                                  >
                                    ...
                                    <div style={{ maxHeight: '200px', overflowY: 'auto', marginTop: "10px" }}>
                                      {getFilteredOptions(column.field).map((option, idx) => (
                                        <div key={idx} className="form-check mb-1 form-check-div">
                                          <input
                                            className="form-check-input"
                                            type="checkbox"
                                            id={`filter-${column.field}-${idx}`}
                                            checked={columnFilters[column.field]?.includes(option)}
                                    ...
                                    <div className="d-flex gap-2 mt-2 pt-2 border-top" >
                                      <button
                                        ...
                                        onClick={() => applyColumnFilter(column.field)} style={{ backgroundColor: "#5a6c5b" }}
                                      >
                                        OK
                                      </button>
                                      <button
