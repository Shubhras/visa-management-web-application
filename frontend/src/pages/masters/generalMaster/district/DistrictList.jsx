...
const DistrictList = () => {
  ...
  const fetchDistrictList = () => {
    ...
    dispatch(
      districtList(params, (response, error) => {
        setLoading(false);
        if (response?.statusCode === 200 && response?.status === true) {
          const paginationData = response?.pagination || {};
          ...
          setSelectedRows((prev) =>
            prev.filter((rowId) => data.some((r) => r.uuid === rowId))
          );
        } else {
          setDistrictDataList([]);
          setTableState((prev) => ({
  ...
  const isAllSelected =
    districtDataList.length > 0 &&
    districtDataList.every((i) => selectedRows.includes(i.uuid));
  const goToPage = (page) => {
    if (page >= 1 && page <= tableState.totalPages)
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all district`
        : `${selectedRows.length} selected district`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this district (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const handleExport = () => {
    ...
    setLoadingExport(true);
    dispatch(
      districtExportData(sendPayload, (response, error) => {
        setLoadingExport(false);
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === districtDataList?.length && (
                      <>
                        <button
                          onClick={() => handleSelectAllOrNot("onlySelected")}
                  ...
                  {loading ? (
                    ...
                    districtDataList.map((rowItem, index) => (
                      <tr key={rowItem.uuid}>
                        ...
                        {visibleColumns.includes("countryName") && (
                          <td>
                            <span>{rowItem.countryName}</span>
                          </td>
                        )}
                        {visibleColumns.includes("stateName") && (
                          <td>
                            <span>{rowItem.stateName}</span>
                          </td>
                        )}
                        {visibleColumns.includes("districtName") && (
                          <td>
