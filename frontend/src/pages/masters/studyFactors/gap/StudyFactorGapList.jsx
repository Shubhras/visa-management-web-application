...
import AddEditStudyFactorGapModal from './AddEditStudyFactorGapModal';
import AddImportStudyFactorGapModal from './AddImportStudyFactorGapModal';
import { factorForList, gapGroupList } from "../../../../store/master/studyFactorsMasters/action";
import { representingCountryList } from "../../../../store/master/occupationMaster/action";
import { instituteTypeList, courseLevelList } from "../../../../store/master/instituteMaster/action";
import ResetButton from '../../../../components/comman/ResetButton';

const StudyFactorGapList = () => {
  ...
  useEffect(() => {
    ...
    fetchStudyGapGroupDropdown();
    fetchInstituteTypeDropdown();
  ...
  const fetchStudyGapGroupDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(gapGroupList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        ...
        setFilterDropdownData(prev => ({
          ...prev,
          studyGapGroup: sortedOptions
        }));
      }
    }));
  };
  const fetchInstituteTypeDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(instituteTypeList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        const options = (response.data || []).map(item => ({
          id: item.uuid || item.id,
          name: String(item.name ?? "")
  ...
  const [items] = useState(["Factor For", "Study : GAP Group", " Maximum GAP Accepted (Months)", "Country", "Intitude Type", "Course Level", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["Factor For", "Study : GAP Group", " Maximum GAP Accepted (Months)", "Country", "Intitude Type", "Course Level",]);
  const [ItemsRequired] = useState(["Factor For", "Study : GAP Group", " Maximum GAP Accepted (Months)", "Country", "Intitude Type", "Course Level",]);
  const [countryListData, setCountryListData] = useState([]);

  const [tableColumns] = useState([
    { id: 'factorForName', label: 'Factor For', field: 'factorFor', visible: true, required: false, filterable: true },
    { id: 'studyAgeGroup', label: 'Study : GAP Group', field: 'studyGapGroup', visible: true, required: false, filterable: true },
    { id: 'maximumAgeAccepted', label: 'Maximum GAP Accepted (Months)', field: 'maximumAgeAccepted', visible: true, required: false, filterable: false },
    { id: 'countryName', label: 'Country for Admission', field: 'representingCountry', visible: true, required: false, filterable: true },
    { id: 'instituteType', label: 'Institute Type', field: 'instituteType', visible: true, required: false, filterable: true },
  ...
  const fetchGapList = () => {
    ...
    const params = {
      ...
      studyGapGroup: columnFilters.studyGapGroup.length > 0 ? columnFilters.studyGapGroup : null,
      courseLevel: columnFilters.courseLevel.length > 0 ? columnFilters.courseLevel : null,
      instituteType: columnFilters.instituteType.length > 0 ? columnFilters.instituteType : null,
    };
  ...
  const confirmDelete = () => {
    ...
    const deleteAll =
      ...
      (
        ...
        columnFilters.studyGapGroup.length > 0 ||
        columnFilters.instituteType.length > 0 ||
        columnFilters.courseLevel.length > 0
      );
    const payloadSend = {
      deleteAll: deleteAll,
      ...
      studyGapGroup: columnFilters.studyGapGroup.length > 0 ? columnFilters.studyGapGroup : '',
      instituteType: columnFilters.instituteType.length > 0 ? columnFilters.instituteType : '',
      courseLevel: columnFilters.courseLevel.length > 0 ? columnFilters.courseLevel : '',
      id: deleteAll == true ? "" : sendPayload,
      search: tableState.search || '',
    };
    dispatch(studyFactorGapDelete(payloadSend, (response, error) => {
      if (error) {
  ...
  const handleExport = () => {
    ...
    const fieldMapping = {
      ...
      "Study : GAP Group": "study_age_group",
      "Maximum GAP Accepted (Months)": "maximum_age_accepted",
  ...
  return (
    <>
      <MasterLayout>
        ...
        />
        {showImport && (
          <AddImportStudyFactorGapModal show={showImport} handleClose={handleCloseImport} />)}
        {showDeleteConfirm && (
...
export default StudyFactorGapList;