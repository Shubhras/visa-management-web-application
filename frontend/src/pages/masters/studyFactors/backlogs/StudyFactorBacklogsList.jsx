...
import { factorForList, backlogsGroupList } from "../../../../store/master/studyFactorsMasters/action";
...
const StudyFactorBacklogsList = () => {
  ...
  useEffect(() => {
    ...
    fetchStudyBacklogGroupDropdown();
  }, []);
  ...
  const fetchStudyBacklogGroupDropdown = () => {
    const params = {
      page: 1,
      limit: 2000,
      search: "",
    ...
    dispatch(backlogsGroupList(params, (response, error) => {
      if (response?.statusCode === 200 && response?.status === true) {
        ...
        setFilterDropdownData(prev => ({
          ...prev,
          studyBacklogGroup: sortedOptions
        }));
      }
    }));
  };
  ...
  const [tableColumns] = useState([
    ...
    { id: 'studyBacklogsGroup', label: 'Study : Backlogs Group', field: 'studyBacklogGroup', visible: true, required: false, filterable: true },
  ...
  const fetchDepartmentList = () => {
    ...
    const params = {
      ...
      studyBacklogGroup: columnFilters.studyBacklogGroup.length > 0 ? columnFilters.studyBacklogGroup : null,
  ...
  const confirmDelete = () => {
    ...
    const deleteAll =
      ...
      (
        ...
        columnFilters.studyBacklogGroup.length > 0

      );
    const payloadSend = {
      deleteAll: deleteAll,
      factorFor: columnFilters.factorFor.length > 0 ? columnFilters.factorFor : '',
      studyBacklogGroup: columnFilters.studyBacklogGroup.length > 0 ? columnFilters.studyBacklogGroup : '',
      id: deleteAll == true ? "" : sendPayload,
      search: tableState.search || '',
    };
    dispatch(studyFactorBacklogsDelete(payloadSend, (response, error) => {
      if (error) {
    ...
    dispatch(studyFactorBacklogsExportData(sendPayload, (response, error) => {
      if (error) {
        ...
        if (response?.status === 200) {
          ...
          link.href = url;
          link.download = `StudyFactor:Backlogs.xlsx`;
