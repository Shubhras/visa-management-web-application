...
const CompanyList = () => {
  ...
  const [items] = useState(["Company Type", "Description", "Modified On"]);
  ...
  const handleSelectAll = (e) => {
    ...
    if (checked) {
      setSelectedRows(companyListData.map((Item) => Item.uuid));
    } else {
      setSelectedRows([]);
      setSelectAllOrNot("");
    }
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all company type`
        : `${selectedRows.length} selected company type`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this company type (${message})?`
    );
    setShowDeleteConfirm(true);
  };
  ...
  const handleExport = () => {
    ...
    dispatch(
      companyExportData(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.status === 200) {
            ...
            link.href = url;
            link.download = `CompanyType.xlsx`;
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === companyListData?.length && (
                      <>
                        <button
