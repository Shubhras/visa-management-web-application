...
import { documentNameImportData } from "../../../../store/master/visaProcessMaster/action";
...
const AddImportDocumentNameModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(documentNameImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Document Name skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Document Category","Document Name"],
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Document Category", "Document Name", "Reason"],
                            sheetName: "DocumentName",
                            fileName: "DocumentName",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    ...
                    items: ["Document Category", "Document Name", "Description"],
                    selectedItems: ["Document Category", "Document Name"],
                    ItemsRequired: ["Document Category", "Document Name"]
                }
                } />
            )}
        </>
...
export default AddImportDocumentNameModal;