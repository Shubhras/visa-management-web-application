...
import { documentNameList, documentNameDelete, documentNameExportData, documentCategoryList } from "../../../../store/master/visaProcessMaster/action";
import AddImportDocumentNameModal from './AddImportDocumentNameModal';
import AddEditDocumentNameModal from './AddEditDocumentNameModal';
...
const DocumentNameList = () => {
    ...
    useEffect(() => {
        fetchDocumentCategoryDropdown();
    }, []);

    const fetchDocumentCategoryDropdown = () => {
        const params = {
            page: 1,
            limit: 2000,
            search: "",
        ...
        dispatch(documentCategoryList(params, (response, error) => {
            if (response?.statusCode === 200 && response?.status === true) {
                ...
                setFilterDropdownData({
                    documentCategory: sortedOptions
                });
    ...
    const clearAllOnlyHeaderFilters = () => setColumnFilters({ documentCategory: [] })
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Document Name?");
    ...
    const [items] = useState(["Document Category", "Document Name", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Document Category", "Document Name",]);
    const [ItemsRequired] = useState(["Document Category", "Document Name",]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'documentCategory', label: 'Document Category', field: 'documentCategory', visible: true, required: false, filterable: true },
        { id: 'name', label: 'Document Name', field: 'name', visible: true, required: false, filterable: false },
    ...
    const fetchDepartmentList = () => {
        ...
        const params = {
            ...
            documentCategory: columnFilters.documentCategory.length > 0 ? columnFilters.documentCategory : null,
        };

        dispatch(documentNameList(params, (response, error) => {
            setLoading(false);
            if (response?.statusCode === 200 && response?.status === true) {
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Document Name?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Document Name` : `${selectedRows.length} selected Document Name`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Document Name (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        const deleteAll = selectAllOrNot === "all" && ((tableState.search && tableState.search.trim() !== '') || columnFilters.documentCategory.length > 0);
        const payloadSend = {
            deleteAll: deleteAll,
            documentCategory: columnFilters.documentCategory.length > 0 ? columnFilters.documentCategory : '',
            id: deleteAll == true ? "" : sendPayload,
            search: tableState.search || '',
        };
        dispatch(documentNameDelete(payloadSend, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        const fieldMapping = {
            "Document Category": "document_category_name",
        ...
        dispatch(documentNameExportData(sendPayload, (response, error) => {
            if (error) {
    ...
    return (
        <>
            <MasterLayout>
                                    ...
                                    {loading ? (
                                        ...
                                        departments.map((rowItem, index) => (
                                            <tr key={rowItem.uuid}>
                                                ...
                                                {isColumnVisible('documentCategory') && (
                                                    <td><span>{rowItem.document_category_name}</span></td>
                                                )}
                                                {isColumnVisible('name') && (
                                                    <td><span>{rowItem.document_name}</span></td>
                                                )}
                ...
                />
                {showImport && (
                    <AddImportDocumentNameModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Document Name</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default DocumentNameList;