...
import { paymentToList, paymentToDelete, paymentToExportData } from "../../../../store/master/visaProcessMaster/action";
import AddImportPaymentToModal from './AddImportPaymentToModal';
import AddEditPaymentToModal from './AddEditPaymentToModal';
...
const PaymentToList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Payment To?");
  ...
  const [items] = useState(["Payment To", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["Payment To"]);
  const [ItemsRequired] = useState(["Payment To"]);

  const [tableColumns] = useState([
    { id: 'name', label: 'Payment To', field: 'name', visible: true, required: false },
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this Payment To?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all Payment To` : `${selectedRows.length} selected Payment To`;
    setDeleteConfirmMessage(`Are you sure you want to delete this Payment To (${message})?`);
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    dispatch(paymentToDelete(payloadSend, (response, error) => {
      if (error) {
  ...
  const handleExport = () => {
    ...
    dispatch(paymentToExportData(sendPayload, (response, error) => {
      if (error) {
  ...
  return (
    <>
      <MasterLayout>
        ...
        {/* Import Modal */}
        {showImport && (
          <AddImportPaymentToModal show={showImport} handleClose={handleCloseImport} />
        )}
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div className="modal-content radius-16 bg-base">
                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                  <h1 className="modal-title fs-5">Export Payment To</h1>
                  <button
                    type="button"
                    className="btn-close"
