...
const AddEditProcessSubStatusNameModal = ({ show, handleClose, mode = 'add', rowData = null }) => {
    ...
    useEffect(() => {
        if (show) {
            ...
            fetchRepresentingCountry();
        }
    }, [mode, rowData, show]);

    const fetchRepresentingCountry = () => {
        const params = {
            page: 1,
            limit: 2000,
            search: '',
    ...
    const validateForm = () => {
        ...
        if (!formData.name.trim()) {
            newErrors.name = 'Process Sub-Status Name is required';
            isValid = false;
        }
        if (!formData.processStatus) {
            newErrors.processStatus = 'Process Status Name is required';
            isValid = false;
        }
        if (!formData.country) {
    ...
    const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            const sendPayload = mode === 'edit'
                ? {
                    uuid: formData.uuid,
                    process_sub_status_name: formData.name,
                    process_status_name: formData.processStatus,
                    country: formData.country,
                    visa_main_category: formData.visaMain,
            ...
            const action = mode === 'edit' ? processSubStatusNameEdit : processSubStatusNameAdd;
                            ...
                            <div className="row">
                                ...
                                <div className="col-12 mb-10">
                                    ...
                                    <input
                                        ...
                                        placeholder="Enter process sub-status name"
                                    />
                                    {errors.name && (
                                        <div className="text-danger text-sm mt-1">
                                            {errors.name}
...
export default AddEditProcessSubStatusNameModal;