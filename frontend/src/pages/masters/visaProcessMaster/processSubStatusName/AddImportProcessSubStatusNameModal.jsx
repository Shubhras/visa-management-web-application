...
import { processSubStatusNameImportData } from "../../../../store/master/visaProcessMaster/action";
...
const AddImportProcessSubStatusNameModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(processSubStatusNameImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Process Sub-Status Name skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Country", "Visa Main Category", "Process Status Name", "Process Sub-Status Name"],
                            sheetName: "ProcessSubStatusName",
                            fileName: "ProcessSubStatusName",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Country", "Visa Main Category", "Process Status Name", "Process Sub-Status Name", "Reason"],
                            sheetName: "ProcessSubStatusName",
                            fileName: "ProcessSubStatusName",
                        };
        ...
        <>
                    ...
                    <div className="modal-content radius-16 bg-base">
                        <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                            <h1 className="modal-title fs-5" id="departmentModalLabel">
                                Upload Process Sub-Status Name
                            </h1>
                            <button
                                type="button"
                                className="btn-close"
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "ProcessSubStatusName",
                    items: ["Country", "Visa Main Category", "Process Status Name", "Process Sub-Status Name", "Description"],
                    selectedItems: ["Country", "Visa Main Category", "Process Status Name", "Process Sub-Status Name"],
                    ItemsRequired: ["Country", "Visa Main Category", "Process Status Name", "Process Sub-Status Name"]
                }
                } />
            )}
        </>
...
export default AddImportProcessSubStatusNameModal;