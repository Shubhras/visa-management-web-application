...
import { purposeOfVisitImportData } from "../../../../store/master/visaProcessMaster/action";
...
const AddImportPurposeofVisitModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(purposeOfVisitImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Purpose of Visit skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Purpose of Visit"],
                            sheetName: "PurposeofVisit",
                            fileName: "PurposeofVisit",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Purpose of Visit", "Reason"],
                            sheetName: "PurposeofVisit",
                            fileName: "PurposeofVisit",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "PurposeofVisit",
                    items: ["Purpose of Visit", "Description"],
                    selectedItems: ["Purpose of Visit"],
                    ItemsRequired: ["Purpose of Visit"]
                }
                } />
            )}
        </>
...
export default AddImportPurposeofVisitModal;