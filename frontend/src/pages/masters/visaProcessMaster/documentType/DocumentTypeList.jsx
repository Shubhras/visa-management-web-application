...
import { documentTypeList, documentTypeDelete, documentTypeExportData } from "../../../../store/master/visaProcessMaster/action";
import AddImportDocumentTypeModal from './AddImportDocumentTypeModal';
...
const  DocumentTypeList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Document Type?");
    ...
    const [items] = useState(["Document Type", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Document Type"]);
    const [ItemsRequired] = useState(["Document Type"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Document Type', field: 'name', visible: true, required: false },
    ...
    const fetchDepartmentList = () => {
        ...
        dispatch(documentTypeList(params, (response, error) => {
            setLoading(false);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Document Type?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Document Type` : `${selectedRows.length} selected Document Type`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Document Type (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Document Type selected for deletion.");
            return;
        }
        dispatch(documentTypeDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(documentTypeExportData(sendPayload, (response, error) => {
            if (error) {
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportDocumentTypeModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Document Type</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
