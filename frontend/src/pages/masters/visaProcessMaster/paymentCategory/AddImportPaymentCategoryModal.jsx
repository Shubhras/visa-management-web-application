...
import { paymentCategoryImportData } from "../../../../store/master/visaProcessMaster/action";
...
const AddImportPaymentCategoryModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(paymentCategoryImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Payment Category skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Payment To", "Payment Category"],
                            sheetName: "PaymentCategory",
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "PaymentCategory",
                    items: ["Payment To","Payment Category", "Description"],
                    selectedItems: ["Payment To","Payment Category"],
                    ItemsRequired: ["Payment To","Payment Category"]
                }
                } />
            )}
        </>
...
export default AddImportPaymentCategoryModal;