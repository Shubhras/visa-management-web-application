...
import { documentsForList, documentsForDelete, documentsForExportData } from "../../../../store/master/visaProcessMaster/action";
import AddImportDocumentsForModal from './AddImportDocumentsForModal';
import AddEditDocumentsForModal from './AddEditDocumentsForModal';
...
const  DocumentsForList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Documents For?");
    ...
    const [items] = useState(["Documents For", "Description", "Modified On"]);
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Documents For?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Documents For` : `${selectedRows.length} selected Documents For`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Documents For (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Documents For selected for deletion.");
            return;
        }
        dispatch(documentsForDelete(sendPayload, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(documentsForExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `DocumentsFor.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportDocumentsForModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Documents For</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default DocumentsForList;