...
import { processTypeList, processTypeDelete, processTypeExportData } from "../../../../store/master/visaProcessMaster/action";
import AddImportProcessTypeModal from './AddImportProcessTypeModal';
import AddEditProcessTypeModal from './AddEditProcessTypeModal';
...
const ProcessTypeList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Process Type?");
  ...
  const [items] = useState(["Process Type", "Description", "Modified On"]);
  const [selectedItems, setSelectedItems] = useState(["Process Type"]);
  const [ItemsRequired] = useState(["Process Type"]);

  const [tableColumns] = useState([
    { id: 'name', label: 'Process Type', field: 'name', visible: true, required: false },
  ...
  const fetchDepartmentList = () => {
    ...
    dispatch(processTypeList(params, (response, error) => {
      setLoading(false);
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this Process Type?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all Process Type` : `${selectedRows.length} selected Process Type`;
    setDeleteConfirmMessage(`Are you sure you want to delete this Process Type (${message})?`);
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    dispatch(processTypeDelete(payloadSend, (response, error) => {
      if (error) {
  ...
  const handleExport = () => {
    ...
    dispatch(processTypeExportData(sendPayload, (response, error) => {
      if (error) {
        ...
        if (response?.status === 200) {
          ...
          link.href = url;
          link.download = `ProcessType.xlsx`;
  ...
  return (
    <>
      <MasterLayout>
        ...
        {/* Import Modal */}
        {showImport && (
          <AddImportProcessTypeModal show={showImport} handleClose={handleCloseImport} />
        )}
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
              <div className="modal-content radius-16 bg-base">
                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                  <h1 className="modal-title fs-5">Export Process Type</h1>
                  <button
                    type="button"
                    className="btn-close"
