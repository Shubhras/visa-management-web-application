...
import { processStatusNameImportData } from "../../../../store/master/visaProcessMaster/action";
...
const AddImportProcessStatusNameModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(processStatusNameImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Process Status Name skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.duplicates?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Country", "Visa Main Category", "Process Status Name"],
                            sheetName: "ProcessStatusName",
                            fileName: "ProcessStatusName",
                        };
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Country", "Visa Main Category", "Process Status Name", "Reason"],
                            sheetName: "ProcessStatusName",
                            fileName: "ProcessStatusName",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "ProcessStatusName",
                    items: ["Country", "Visa Main Category", "Process Status Name", "Description"],
                    selectedItems: ["Country", "Visa Main Category", "Process Status Name"],
                    ItemsRequired: ["Country", "Visa Main Category", "Process Status Name"]
                }
                } />
            )}
        </>
...
export default AddImportProcessStatusNameModal;