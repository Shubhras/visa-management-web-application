...
import { processStatusNameList, processStatusNameDelete, processStatusNameExportData } from "../../../../store/master/visaProcessMaster/action";
import AddImportProcessStatusNameModal from './AddImportProcessStatusNameModal';
import AddEditProcessStatusNameModal from './AddEditProcessStatusNameModal';
...
const ProcessStatusNameList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Process Status Name?");
    ...
    const [items] = useState(["Country", "Visa Main Category", "Process Status Name", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Country", "Visa Main Category", "Process Status Name"]);
    const [ItemsRequired] = useState(["Country", "Visa Main Category", "Process Status Name"]);
    ...
    const [tableColumns] = useState([
        ...
        { id: 'visaMajor', label: 'Process Status Name', field: 'visaMajor', visible: true, required: false, filterable: false },
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Process Status Name?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Process Status Name` : `${selectedRows.length} selected Process Status Name`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Process Status Name (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        const deleteAll = selectAllOrNot === "all" && ((tableState.search && tableState.search.trim() !== '') || columnFilters.representingCountry.length > 0 || columnFilters.visaMainArea.length > 0);
        const payloadSend = {
            deleteAll: deleteAll,
        ...
        dispatch(processStatusNameDelete(payloadSend, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(processStatusNameExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `ProcessStatusName.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportProcessStatusNameModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Process Status Name</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
...
export default ProcessStatusNameList;