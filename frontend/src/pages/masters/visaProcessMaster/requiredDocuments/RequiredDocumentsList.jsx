...
import { requiredDocumentGeneralList, requiredDocumentGeneralDelete, requiredDocumentGeneralExportData, documentsForList } from "../../../../store/master/visaProcessMaster/action";
import AddImportRequiredDocumentsModal from './AddImportRequiredDocumentsModal';
import AddEditRequiredDocumentsModal from './AddEditRequiredDocumentsModal';
...
import { visaMajorCategoryList } from '../../../../store/master/visaMaster/action';
...
const RequiredDocumentsList = () => {
    ...
    useEffect(() => {
        ...
        fetchVisaMajorAreaDropdown();
        fetchRepresentingCountryDropdown();
        fetchDocumentForDropdown();
    }, []);
    ...
    const fetchVisaMajorAreaDropdown = () => {
        const params = {
            page: 1,
            limit: 2000,
            search: "",
    ...
    const fetchDocumentForDropdown = () => {
        const params = {
            page: 1,
            limit: 2000,
            search: "",
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Required Documents (General)?");
    ...
    const [items] = useState(["Documents For", "Country", "Visa Main Category", "Visa Major Category", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Documents For"]);
    const [ItemsRequired] = useState(["Documents For"]);

    // Table columns configuration

    const [tableColumns] = useState([
        { id: 'name', label: 'Documents For', field: 'documentFor', visible: true, required: false, filterable: true },
        ...
        { id: 'visaMajor', label: 'Visa Major Category', field: 'visaMajorArea', visible: true, required: false, filterable: true },
    ...
    const fetchDepartmentList = () => {
        ...
        const params = {
            ...
            documentFor: columnFilters.documentFor.length > 0 ? columnFilters.documentFor : null,
            ...
            visaMajorArea: columnFilters.visaMajorArea.length > 0 ? columnFilters.visaMajorArea : null,
        };

        dispatch(requiredDocumentGeneralList(params, (response, error) => {
            setLoading(false);
            if (response?.statusCode === 200 && response?.status === true) {
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Required Documents (General)?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Required Documents (General)` : `${selectedRows.length} selected Required Documents (General)`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Required Documents (General) (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        const deleteAll = selectAllOrNot === "all" && ((tableState.search && tableState.search.trim() !== '') || columnFilters.documentFor.length > 0 || columnFilters.representingCountry.length > 0 || columnFilters.visaMainArea.length > 0 || columnFilters.visaMajorArea.length > 0);
        const payloadSend = {
            deleteAll: deleteAll,
            documentFor: columnFilters.documentFor.length > 0 ? columnFilters.documentFor : '',
            ...
            visaMajorArea: columnFilters.visaMajorArea.length > 0 ? columnFilters.visaMajorArea : '',
            id: deleteAll == true ? "" : sendPayload,
            search: tableState.search || '',
        };
        dispatch(requiredDocumentGeneralDelete(payloadSend, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(requiredDocumentGeneralExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `RequiredDocuments(General).xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportRequiredDocumentsModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Required Documents (General)</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
