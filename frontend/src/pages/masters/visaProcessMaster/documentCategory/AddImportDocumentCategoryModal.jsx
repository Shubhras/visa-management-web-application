...
import { documentCategoryImportData } from "../../../../store/master/visaProcessMaster/action";
...
const AddImportDocumentCategoryModal = ({ show, handleClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setLoading(true);
        dispatch(documentCategoryImportData(formData, (response, error) => {
            setLoading(false);
            if (error) {
                ...
                if (response?.statusCode === 200 && response?.status === true) {
                        ...
                        <div>
                            ...
                            {response?.duplicates?.length > 0 && (
                                <div style={{ marginTop: '6px' }}>
                                    <strong>Duplicate Document Category skipped — the duplicate data from your uploaded file has been exported into an .xlsx file.</strong>
                                </div>
                            )}
                        </div>,
                        {
                    ...
                    if (response?.skipped_rows?.length > 0) {
                        const prepareData = {
                            ...
                            headers: ["Document Category", "Reason"],
                            sheetName: "DocumentCategory",
                            fileName: "DocumentCategory",
                        };
        ...
        <>
            ...
            {showSampleExcelDownload && (
                <CommanSampleExcelDownloadModal show={showSampleExcelDownload} handleClose={handleCloseSampleExcelDownload} prepareData={{
                    downloadFileName: "DocumentCategory",
                    items: ["Document Category", "Description"],
                    selectedItems: ["Document Category"],
                    ItemsRequired: ["Document Category"]
                }
                } />
            )}
        </>
...
export default AddImportDocumentCategoryModal;