...
import { documentCategoryList, documentCategoryDelete, documentCategoryExportData } from "../../../../store/master/visaProcessMaster/action";
import AddImportDocumentCategoryModal from './AddImportDocumentCategoryModal';
import AddEditDocumentCategoryModal from './AddEditDocumentCategoryModal';
...
const DocumentCategoryList = () => {
    ...
    const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this Document Category?");
    ...
    const [items] = useState(["Document Category", "Description", "Modified On"]);
    const [selectedItems, setSelectedItems] = useState(["Document Category"]);
    const [ItemsRequired] = useState(["Document Category"]);

    // Table columns configuration
    const [tableColumns] = useState([
        { id: 'name', label: 'Document Category', field: 'name', visible: true, required: false },
    ...
    const handleDelete = (uuid) => {
        ...
        setDeleteConfirmMessage(`Are you sure you want to delete this Document Category?`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        const message = selectAllOrNot === "all" ? `${tableState.total} all Document Category` : `${selectedRows.length} selected Document Category`;
        setDeleteConfirmMessage(`Are you sure you want to delete this Document Category (${message})?`);
        setShowDeleteConfirm(true);
    };
    ...
    const confirmDelete = () => {
        ...
        if (!sendPayload || sendPayload.length === 0) {
            toast.error("No Document Category selected for deletion.");
            return;
        }
        ...
        dispatch(documentCategoryDelete(payloadSend, (response, error) => {
            if (error) {
    ...
    const handleExport = () => {
        ...
        dispatch(documentCategoryExportData(sendPayload, (response, error) => {
            if (error) {
                ...
                if (response?.status === 200) {
                    ...
                    link.href = url;
                    link.download = `DocumentCategory.xlsx`;
    ...
    return (
        <>
            <MasterLayout>
                ...
                />
                {showImport && (
                    <AddImportDocumentCategoryModal show={showImport} handleClose={handleCloseImport} />)}
                {showDeleteConfirm && (
                        ...
                        <div className="modal-dialog modal-xl modal-dialog-centered" role="document">
                            <div className="modal-content radius-16 bg-base">
                                <div className="modal-header py-16 px-24 border border-top-0 border-start-0 border-end-0">
                                    <h1 className="modal-title fs-5">Export Document Category</h1>
                                    <button
                                        type="button"
                                        className="btn-close"
