...
import { ownershipTypeList, ownershipTypeDelete, ownershipTypeExportData } from '../../../../store/master/companyMasters/actions';
...
const OwnershipTypeList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState("Are you sure you want to delete this ownership type?");
  ...
  const [tableColumns] = useState([
    { id: 'name', label: 'Ownership Type', field: 'name', visible: true, required: false },
    { id: 'company_type_name', label: 'Company Type', field: 'company_type_name', visible: true, required: false },
  ...
  const fetchBankAccountTypeList = () => {
    ...
    dispatch(ownershipTypeList(params, (response, error) => {
      setLoading(false);
  ...
  const isAllSelected = ownershipTypeData.length > 0 &&
  ...
  const handleDelete = (uuid) => {
    ...
    setDeleteConfirmMessage(`Are you sure you want to delete this ownership type?`);
  };
  ...
  const handleBulkDelete = () => {
    ...
    const message = selectAllOrNot === "all" ? `${tableState.total} all ownership type` : `${selectedRows.length} selected ownership type`;
    setDeleteConfirmMessage(`Are you sure you want to delete this ownership type (${message})?`);
    setShowDeleteConfirm(true);
  };
  ...
  const confirmDelete = () => {
    ...
    dispatch(ownershipTypeDelete(sendPayload, (response, error) => {
      if (error) {
        toast.error(error?.response?.data?.message || "server error");
      } else {
        if (response?.statusCode === 200 && response?.status === true) {
          toast.success(response?.message);
          setOwnershipTypeData(prevRowItems => prevRowItems.filter(Item => Item.uuid !== deleteId));
  ...
  const handleExport = () => {
    ...
    dispatch(ownershipTypeExportData(sendPayload, (response, error) => {
      if (error) {
  ...
  return (
    <>
      <MasterLayout>
            ...
            <div className="row align-items-center gy-3 gx-2 flex-wrap filter-action-btn">
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {(selectedRows?.length > 0 && selectedRows?.length === ownershipTypeData?.length) && (
                    <>
                      <button
                                ...
                                ownershipTypeData.map((rowItem, index) => (
                                  <tr key={rowItem.uuid}>
                                    ...
                                    {isColumnVisible('company_type_name') && (
                                      <td><span>{rowItem.company_type_name}</span></td>
                                    )}
        ...
        />
        {showImport && (
          <AddImportOwnershipTypeModal show={showImport} handleClose={handleCloseImport} />)}
        {showDeleteConfirm && (
