...
const LicenceNameList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState(
    "Are you sure you want to delete this licence name?"
  );
  ...
  const [tableColumns] = useState([
    ...
    },
    {
      ...
      label: "License Issuing Authority Name",
    ...
    },
    {
      ...
      label: "License Valid Duration Value",
    ...
    },
    {
      ...
      label: "License Valid Duration Unit",
  ...
  const fetchLicenceNameList = () => {
    ...
    dispatch(
      licenceNameList(params, (response, error) => {
        setLoading(false);
        if (response?.statusCode === 200 && response?.status === true) {
          ...
          // Keep only still-visible selected rows
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all licence name`
        : `${selectedRows.length} selected licence name`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this licence name (${message})?`
    );
    setShowDeleteConfirm(true);
  };
      ...
      licenceNameDelete(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.statusCode === 200 && response?.status === true) {
            ...
            setLicenceNameData((prevRowItems) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      licenceNameExportData(sendPayload, (response, error) => {
        if (error) {
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === licenceNameData?.length && (
                      <>
                        <button
