...
import {
  accreditationCategoryExportData,
...
const AccrediationCategoryList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState(
    "Are you sure you want to delete this accrediation category?"
  );
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all accrediation category`
        : `${selectedRows.length} selected accrediation category`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this accrediation category (${message})?`
    );
    setShowDeleteConfirm(true);
  };
      ...
      accreditationCategoryDelete(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.statusCode === 200 && response?.status === true) {
            ...
            setAccrediationCategoryData((prevRowItems) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      accreditationCategoryExportData(sendPayload, (response, error) => {
        if (error) {
  ...
  return (
    <>
      <MasterLayout>
        ...
        />
        {showImport && (
          <AddImportAccrediationCategoryModal
            show={showImport}
