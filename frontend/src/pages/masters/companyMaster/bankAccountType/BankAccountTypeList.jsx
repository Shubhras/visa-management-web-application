...
import {
  bankAccountTypeList,
  bankAccountTypeDelete,
  bankAccountTypeExportData,
...
const BankAccountTypeList = () => {
  ...
  const [deleteConfirmMessage, setDeleteConfirmMessage] = useState(
    "Are you sure you want to delete this bank account type?"
  );
  ...
  const fetchBankAccountTypeList = () => {
    ...
    dispatch(
      bankAccountTypeList(params, (response, error) => {
        setLoading(false);
  ...
  const handleBulkDelete = () => {
    ...
    const message =
      selectAllOrNot === "all"
        ? `${tableState.total} all bank account type`
        : `${selectedRows.length} selected bank account type`;
    setDeleteConfirmMessage(
      `Are you sure you want to delete this bank account type (${message})?`
    );
    setShowDeleteConfirm(true);
  };
      ...
      bankAccountTypeDelete(sendPayload, (response, error) => {
        if (error) {
          ...
          if (response?.statusCode === 200 && response?.status === true) {
            ...
            setBankAccountTypeData((prevRowItems) =>
  ...
  const handleExport = () => {
    ...
    dispatch(
      bankAccountTypeExportData(sendPayload, (response, error) => {
        if (error) {
              ...
              <div className="col-xl-6 col-lg-4 col-md-12">
                <div className="d-flex flex-wrap align-items-center gap-2">
                  ...
                  {selectedRows?.length > 0 &&
                    selectedRows?.length === bankAccountTypeData?.length && (
                      <>
                        <button
