...
class LeadDocumentSerializer(serializers.Serializer):
    ...
    documentcategory = serializers.UUIDField()
    documentname = serializers.UUIDField()
    ...
    def validate(self, data):
        ...
        from lead.models import Applicant, DocumentCategory, DocumentName

        if not Applicant.objects.filter(uuid=data["applicant"]).exists():
            raise serializers.ValidationError({"applicant": "Invalid applicant UUID"})

        if not DocumentCategory.objects.filter(uuid=data["documentcategory"]).exists():
            raise serializers.ValidationError({"documentcategory": "Invalid document category UUID"})

        if not DocumentName.objects.filter(uuid=data["documentname"]).exists():
